...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://microbiologyjournal.org"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_553.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("div", id="td-header-menu",role="navigation").find("ul", class_="sub-menu").find("a", string="Current Issue").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="uplp-list", id="uplp-list-94544").findAll("li")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="uplp-post-title")
                        if article_title_div:
                            article_title = article_title_div.find("a").text.strip()
                            article_link = article_title_div.find("a").get('href')

                            doi_tag = single_element.find("div", class_="uplp-post-excerpt").findAll("p")[1]
                            doi = ""
                            if doi_tag:
                                doi = doi_tag.get_text(strip=True).replace("https://doi.org/", "") if doi_tag else ""

                            pdf_url_text = single_element.find("div", class_="uplp-post-excerpt").findAll("p")[2]
                            pdf_link_tag = pdf_url_text.find("a", href=True, string=" PDF")
                            pdf_link = ""
                            if pdf_link_tag:
                            ...
                            volume_issue_date_info = soup.find("div", class_="vc_custom_heading vc_custom_1725285335363")
                            volume, issue, month, year = "", "", "", ""
                            if volume_issue_date_info:
                                volume_issue_text = volume_issue_date_info.get_text(strip=True)
                                match = re.search(r"Volume (\d+) Issue (\d+), (\w+) (\d+)", volume_issue_text)
                                if match:
                            ...
                            if article_response.status_code == 200:
                                ...
                                pages_tag = article_soup.find("div", class_="td-fix-index").findAll("div", class_="emailtag")[1]
                                page_range = ""
                                if pages_tag:
                                    pages_text = pages_tag.get_text(strip=True)
                                    match = re.search(r":(\d+-\d+)\.", pages_text)
                                    if match:
                                        page_range = match.group(1) if pages_text else ""
                                    else:
