...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.cjcu.jlu.edu.cn/EN"
            toc_url = "http://www.cjcu.jlu.edu.cn/EN/0251-0790/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_576.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").findAll("ul", class_="dropdown-menu")[1].find("a", string="Current Issue", role="menuitem").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="articles").findAll("div", class_="wenzhang")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("dl")
                        if article_title_div:
                            article_title = article_title_div.find("dd").find("a", class_="biaoti_en").text.strip()
                            article_link = article_title_div.find("dd").find("a", class_="biaoti_en").get('href')
                            ...
                            if page_ranges_doi_tag:
                                ...
                                page_range_match = re.search(r"(\d+-\d+)|(\d{8})", page_ranges_doi_text)
                                if page_range_match:
                                    page_range = page_range_match.group(0) if page_range_match else ""
                                else:
                                    print("Page Range not found.")

                                doi_match = re.search(r"doi:\S+", page_ranges_doi_text)
                                if doi_match:
                                    doi = doi_match.group(0).replace("doi:", "") if doi_match else ""
                                # else:
                                #     print("DOI not found.")

                            date_volume_issue_info = soup.find("div", class_="njq")
                            ...
                            if date_volume_issue_info:
                                ...
                                if date_volume_issue_match:
                                    day = date_volume_issue_match.group(1) if date_volume_issue_match else ""
                                    month = date_volume_issue_match.group(2) if date_volume_issue_match else ""
                                    year = date_volume_issue_match.group(3) if date_volume_issue_match else ""
                                    volume = date_volume_issue_match.group(4) if date_volume_issue_match else ""
                                    issue = date_volume_issue_match.group(5) if date_volume_issue_match else ""

                            token = get_token("http://www.cjcu.jlu.edu.cn/EN/0251-0790/home.shtml")
                            pdf_link = f"http://www.cjcu.jlu.edu.cn/EN/PDF/{doi}?token={token}"
