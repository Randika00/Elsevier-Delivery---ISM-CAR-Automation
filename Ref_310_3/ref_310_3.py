...
try:
    ...
    for line in url_details:
        try:
            ...
            toc_url = "https://riviste.unimi.it/index.php/RIPS/user/setLocale/en_US?source=%2Findex.php%2FRIPS%2Fissue%2Farchive"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_310_3.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", id="navigationPrimary", class_="pkp_navigation_primary pkp_nav_list").findAll("li")[3].find("a").get('href')
            current_link = f"https://riviste.unimi.it/index.php/RIPS/user/setLocale/en_US?source=%2Findex.php%2FRIPS%2Fissue%2Fcurrent"
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if month_text:
                                ...
                                month_match = re.search(r'(\d{2}-\d{2}-\d{4})', published_date_text)
                                if month_match:
                            ...
                            if volume_year_info:
                                ...
                                match = re.search(r'Vol\.\s*(\d+)\s*No\.\s*(\d+)\s*\((\d{4})\)', volume_year_text)
                                if match:
                                    volume = match.group(1)
                                    issue = match.group(2)
                                    year = match.group(3)
                            else:
                                print("volume, issue, year not found")
