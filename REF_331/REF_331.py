...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_331.ini")
        ...
        date_time_meta=soup.find("li",class_="banMs").get_text(strip=True)
        match = re.search(r'(\d{4})年第(\d+)卷第(\d+)期', date_time_meta)
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://cjb.ijournals.cn/" + All_articles[article_index].find("div", class_="slideTxtBox_list_title").find("a").get("href")

                doi = All_articles[article_index].find("div", class_="slideTxtBox_list_time").find("a",class_="slide_red").get_text(strip=True)

                pdf_page_soup=get_soup(Article_link)

                Article_title = pdf_page_soup.find_all("div", class_="title")[1].get_text(strip=True)
                page_range_ele=pdf_page_soup.find("p",id="cp-cont").get_text(strip=True)
                match = re.search(r':\s*([A-Za-z0-9-]+)$', page_range_ele)
                ...
                pdf_link = pdf_page_soup.find("a",class_="pt1").get("href")
                xml_link="https://cjb.ijournals.cn/" +pdf_page_soup.find("a",class_="pt3").get("href")
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    if download_successful:
                        ...
                        data.append({
                            ...
                            "XML File Name": f"{pdf_count}.xml","user_id": user_id,"TOC_Name":TOC_name
                        })
