...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://ch.whu.edu.cn"
            toc_url = "http://ch.whu.edu.cn/en/article/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_539.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav-inner clearfix container").findAll("li", type={"current_en"})
            current_link = ""
            if link:
                link_text = link[0].find("a")["href"]
                    ...
                    for single_element in div_element:
                        ...
                        try:
                            ...
                            if article_title_div:
                                ...
                                if pages_doi_tag:
                                    page_range_tag = pages_doi_tag.find_all("font")
                                    if page_range_tag:
                                        page_range = page_range_tag[0].text.split(":")[1].strip().rstrip(".") if page_range_tag else ""

                                    doi_tag = pages_doi_tag.find("a")
                                    if doi_tag:
                                        doi = doi_tag.get("href").replace("/en/article/doi/", "") if doi_tag else ""
                                ...
                                pdf_url = f"http://ch.whu.edu.cn/article/exportPdf?id={id_values_string}&language=en"
