...
try:
    ...
    for line in url_details:
        try:
            ...
            toc_url = "https://riviste.unimi.it/index.php/itinera/user/setLocale/en_US?source=%2Findex.php%2Fitinera%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_310_4.ini")
            ...
            current_link = f"https://riviste.unimi.it/index.php/itinera/user/setLocale/en_US?source=%2Findex.php%2Fitinera%2Fissue%2Fcurrent"
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            a_tag = article_title_div.find("a")
                            if a_tag:
                                article_link = a_tag["href"]
                                article_title = "".join(a_tag.find_all(string=True, recursive=False)).strip()
                                ...
                                last_page_link = ""
                                if last_page_text:
                                    last_page_link = last_page_text.find("a", class_="obj_galley_link pdf").get(
                                        'href') if last_page_text else ""
                                else:
                                ...
                                if article_response.status_code == 200:
                                    ...
                                    doi_tag = article_soup.find("section", class_="item doi").find("span",
                                                                                                   class_="value")
                                    doi = ""
                                    if doi_tag:
