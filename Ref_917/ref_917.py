...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.nogr.org/jour/user/setLocale"
            toc_url = "https://www.nogr.org/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_917.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", id="navbar").find("li", id="current").find("a", string="Current").get("href")
            link_id = link.split('/')[-1]

            current_link = f"https://www.nogr.org/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2F{link_id}"
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            issue_year_info = soup.find("div", class_="tocArticle flex").find("div", class_="title")
                            issue, year = "", ""
                            if issue_year_info:
                                issue_year_text = issue_year_info.text.strip()

                                volume_match = re.search(r"Vol\s+(\d+)", issue_year_text)
                                issue_match = re.search(r"No\s+([\d-]+)", issue_year_text)
                                year_match = re.search(r"\((\d{4})\)", issue_year_text)
                            ...
                            if article_response.status_code == 200:
                                ...
                                doi_tags = article_soup.find("div", id="content").find_all("a", id=lambda x: x and "pub-id::doi" in x)
                                doi = ""
                                if doi_tags:
                                    doi = doi_tags[0].get('href').replace("https://doi.org/", "") if doi_tags else ""
                                # else:
