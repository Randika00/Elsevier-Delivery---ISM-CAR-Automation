...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.yxyqc.net/EN/"
            toc_url = "http://www.yxyqc.net/EN/1673-8926/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_358.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("div", class_="center-box-l").find("ul", class_="zxydpt").findAll("li")[1].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="info_cont_two current").findAll("dl")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("span", class_="title")
                        if article_title_div:
                            article_title = article_title_div.find("a", class_="text-title").text.strip()
                            article_link = article_title_div.find("a", class_="text-title").get('href')

                            year_volume_issue_pages_info = single_element.findAll("p")[1]
                            year, volume, issue, page_range = "", "", "", ""
                            if year_volume_issue_pages_info:
                                text = year_volume_issue_pages_info.text.strip()

                                year_match = re.search(r"(\d{4})", text)
                                volume_issue_match = re.search(r"Vol\.(\d+)\((\d+)\)", text)
                                page_range_match = re.search(r":\s+(\d+–\d+)", text)

                                if year_match and volume_issue_match and page_range_match:
                                ...
                                doi = ""
                                if doi_tag:
                                    doi_link = doi_tag.find("a").get("href")
                                    doi = doi_link.split("doi.org/")[1] if doi_tag else ""

                                month_info = soup.find("div", class_="book-content current").find("p", class_="book-date")
                                month = ""
                                if month_info:
                                    published_date = month_info.text.strip()
                                    month_match = re.search(r"Published:\d{2} (\w+) \d{4}", published_date)
                                    if month_match:
                                        month = month_match.group(1) if month_match else ""

                                    onclick_attr = single_element.find_all("a", class_="J_WenZhang_U")
                                    pdf_id = ""
                                    for tag in onclick_attr:
                                        onclick_value = tag.get("onclick", "")
                                        pdf_id_match = re.search(r"lsdy1\('PDF','(\d+)'", onclick_value)
                                        if pdf_id_match:
                                            pdf_id = pdf_id_match.group(1) if pdf_id_match else None

                                        base_pdf_url = "http://www.yxyqc.net/EN/article/downloadArticleFile.do?attachType=PDF&id="
