...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://financetp.fa.ru"
            toc_url = "https://financetp.fa.ru/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_1401.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", id="mobile-menu").find("li", id="current").find("a").get('href')
            link_id = link.split('/')[-1]

            current_link = f"https://financetp.fa.ru/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2F{link_id}"
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            pages_text = single_element.find("span", class_="pages grey")
                            page_range = ""
                            if pages_text:
                                page_range = pages_text.text.strip()

                            volume_issue_year_info = soup.find("div", class_="tocArticle flex").find("div", class_="title")
                            volume, issue, year = "", "", ""
                            if volume_issue_year_info:
                                volume_issue_year_text = volume_issue_year_info.text.strip()
                                match = re.search(r'Vol (\d+), No (\d+) \((\d+)\)', volume_issue_year_text)
                                if match:
                            ...
                            pdf_link_text = single_element.find("span", class_="galleys")
                            last_page_link = ""
                            if pdf_link_text:
                                last_page_link = pdf_link_text.findAll("a", class_="file")[1].get('href')
                                ...
                                pdf_url_info = last_soup.find("div", id="pdfDownloadLinkContainer")
                                pdf_link = ""
                                if pdf_url_info:
                                    pdf_link = pdf_url_info.find("a", class_="action pdf", id="pdfDownloadLink").get('href') if pdf_url_info else ""

                                article_response = get_response_with_retry(article_link, headers=headers)
                                if article_response.status_code == 200:
                                    article_soup = BeautifulSoup(article_response.content, 'html.parser')

                                    doi_tag = article_soup.find("div", class_="reference")
                                    doi = ""
                                    if doi_tag:
                                        doi_paragraph = doi_tag.find("p", id="citEng")
                                        doi = doi_paragraph.find("a").get('href').replace("https://doi.org/", "") if doi_tag else ""
