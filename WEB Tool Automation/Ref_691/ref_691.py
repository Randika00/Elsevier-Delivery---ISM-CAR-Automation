...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://jcspe.speri.com.cn/EN"
            toc_url = "https://jcspe.speri.com.cn/EN/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_691.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("div", class_="item-content").findAll("li", class_="col-md-3 col-sm-3 col-xs-6 text-center")[1].find("a").get("href")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            latest_issue_element = soup.find("span", class_="n-j-q")
                            day, month, year, volume, issue = "", "", "", "", ""
                            if latest_issue_element:
                                latest_issue_text = latest_issue_element.get_text(strip=True)

                                match = re.search(r"(\d{4}) Volume (\d+) Issue (\d+)Published: (\d+) (\w+) (\d{4})", latest_issue_text)

                                if match:
                                    year = match.group(1) if latest_issue_text else ""
                                    volume = match.group(2) if latest_issue_text else ""
                                    issue = match.group(3) if latest_issue_text else ""
                                    day = match.group(4) if latest_issue_text else ""
                                    month = match.group(5) if latest_issue_text else ""

                                else:
                                    print("Could not extract publication details.")

                                token = get_token("https://jcspe.speri.com.cn/")
                                pdf_link = f"https://jcspe.speri.com.cn/EN/PDF/{doi}?token={token}"
