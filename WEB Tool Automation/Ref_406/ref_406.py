...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.cmemo.org.cn/EN"
            toc_url = "http://www.cmemo.org.cn/EN/1004-132X/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_406.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").findAll("li", {"role": "presentation"})[6].find("a", {"role": "menuitem"}).get("href")
            ...
            if response.status_code == 200:
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="wenzhang")
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                page_range_match = re.search(r'\d{4}, \d{2}\(\d+\):\s*([\d,-]+)', page_ranges_doi_text)
                                if page_range_match:
                                    page_range = page_range_match.group(1).strip() if page_range_match else ""

                                doi_match = re.search(r'DOI:\s*(10\.\d{4,9}/[^\s]+)', page_ranges_doi_text)
                                if doi_match:
                            ...
                            if volume_issue_info:
                                ...
                                if match:
                                    ...
                                    print("Could not extract volume & issue.")
                            ...
                            if day_month_year_info:
                                ...
                                if date_match:
                                    day = date_match.group(1) if day_month_year_text else ""
                                    month = date_match.group(2) if day_month_year_text else ""
                                    year = date_match.group(3) if day_month_year_text else ""

                                else:
                                    print("Could not extract publication details.")

                            token = get_token("http://www.cmemo.org.cn//")
                            pdf_link = f"http://www.cmemo.org.cn/EN/PDF/{doi}?token={token}"

                            check_value, tpa_id = common_function.check_duplicate(doi, article_title, source_id, volume,
                                                                                  issue)
