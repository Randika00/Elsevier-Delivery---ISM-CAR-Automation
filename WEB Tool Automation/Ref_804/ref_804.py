...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.china-simulation.com/EN"
            toc_url = "https://www.china-simulation.com/EN/1004-731X/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_804.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").findAll("ul", class_="dropdown-menu")[1].find("li", attrs={"role": "presentation"}).find("a", attrs={"role": "menuitem"}).get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="c_nr").findAll("div", class_="dqml row")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="dqml_title")
                        if article_title_div:
                            ...
                            pages_doi_tag = single_element.findAll("div", class_="dqml_zaiyao")[1]

                            page_range_match = re.search(r"\d{4}-\d{4}", pages_doi_tag.text)
                            page_range, doi = "", ""
                            if page_range_match:
                                page_range = page_range_match.group(0) if page_range_match else ""
                            else:
                                print("Page range not found")

                            doi_link = pages_doi_tag.find("a", href=re.compile(r"dx\.doi\.org"))
                            if doi_link:
                                doi = doi_link.text.strip().replace("http://dx.doi.org/", "") if doi_link else ""
                            else:
                                print("DOI not found")

                            volume_issue_date_text = soup.find("div", class_="fmsm_njq")
                            volume, day, month, year = "", "", "", ""
                            if volume_issue_date_text:
                                if volume_issue_date_text:
                                    full_text = volume_issue_date_text.text.strip()

                                    volume_match = re.search(r"Volume\s+(\d+)", full_text)
                                    if volume_match:
                                        volume = volume_match.group(1) if full_text else ""

                                    text_after_br = volume_issue_date_text.decode_contents().split("<br/>")[1].strip()

                                    date_match = re.match(r"(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})", text_after_br)
                                    if date_match:
                                        day, month, year = date_match.groups() if date_match else ""

                            issue_text = single_element.findAll("div", class_="dqml_zaiyao")[1]
                            issue = ""
                            if issue_text:
                                issue_text_content = issue_text.get_text(strip=True)  # Extract the text
                                issue_match = re.search(r"\((\d+)\)", issue_text_content)
                                if issue_match:
                                    issue = issue_match.group(1) if issue_match else ""
                                else:
                                    print("Issue number not found.")

                            token = get_token("https://www.china-simulation.com/EN/1004-731X/home.shtml")
                            pdf_link = f"https://www.china-simulation.com/EN/PDF/{doi}?token={token}"
