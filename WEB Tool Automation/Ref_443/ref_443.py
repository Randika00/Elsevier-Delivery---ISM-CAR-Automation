...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://lxsj.cstam.org.cn"
            toc_url = "https://lxsj.cstam.org.cn/en/article/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_443.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav-inner clearfix container").find("li", attrs={"type": "/cn/article/current"}).find("a").get("href")

            home_link = base_url + link
            current_link = home_link.replace('/cn', '/en')
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="issue_list").findAll("div", class_="article-list")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            article_links = article_title_div.find("a").get('href') if article_title_div else ""
                            ...
                            if pages_doi_tag:
                                ...
                                doi = pages_doi_tag.find("a").get("href").replace("/en/article/doi/",
                                                                                  "") if pages_doi_tag else ""

                                onclick_attr = single_element.find("div",
                                                                   class_="article-list-zy article-list-btn clear").find_all(
                                    "a")
                                filtered_links = filter(lambda link: link.get("onclick"), onclick_attr)
                                id_values = list(
                                    map(lambda link: re.search(r"downloadpdf\('(.+?)'\)", link.get("onclick")).group(1),
                                        filtered_links))
                                id_values_string = " ".join(id_values)
                                pdf_url = f"https://lxsj.cstam.org.cn/article/exportPdf?id={id_values_string}&language=en"
