...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.ije.ir"
            article_tag = "https://www.ije.ir/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_324.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").find("ul", class_="dropdown-menu").find("li").find("a").get("href").replace("./", "/")
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if article_response.status_code == 200:
                                ...
                                doi_text = article_soup.find("div", class_="margin-bottom-3 ltr", id="ar_doi", title="DOI")
                                doi = ""
                                if doi_text:
                                    doi = doi_text.find("span", dir="ltr").text.strip().replace("https://doi.org/", "") if doi_text else ""
