...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://engineeringmechanics.cn"
            toc_url = "http://engineeringmechanics.cn/en/article/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_520.ini")
            ...
            link_tag = soup.find("div", id="archive").find("div", class_="arci-t pull-left").find("a")
            link_text = base_url + link_tag["href"]
            current_link = link_text.replace('/article/2025/3', '/article/current')

            voliss_text = link_tag.find("p", class_="voliss").get_text(strip=True)
            year, issue = voliss_text.replace("Year ", "").replace("Issue ", "").split(", ")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", id="issueList").findAll("div", class_="article-list-right")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            pdf_link = f"http://engineeringmechanics.cn/article/exportPdf?id={id_values_string}&language=en"
                            xml_link = f"http://engineeringmechanics.cn/article/exportXML?ids={id_values_string}&downType=XML&language=en"
                            ...
                            #     try:
                            #         article_response.html.render(sleep=10, timeout=30)
                            #         html_content = article_response.html.html
                            #         soup = BeautifulSoup(html_content, "html.parser")
                            #
                            #         month_tag = soup.find("div", class_="wrapper").find("div", class_="ii-pub-date")
                            #         month = ""
                            #         if month_tag:
                            #             month_short = month_tag.text.strip().split()[0].replace('.', '')
                            #             month = month_mapping.get(month_short, month_short)
                            #     except Exception as e:
                            #         print(f"Rendering failed for {article_link} with error: {str(e)}")
