...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://medicine.dp.ua/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_376.ini")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            article_titles = article_title_div.find("a")
                            article_link = article_title_div.find("a").get('href')

                            subtitle_span = article_titles.find("span", class_="subtitle")
                            if subtitle_span:
                                subtitle_span.extract()

                            article_title = article_titles.get_text(separator=" ", strip=True)
