...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.ryhxgy.cn/EN"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_294.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", class_="col-md-3 col-xs-3").findAll("div", class_="links")[1].findAll("li")[1].find("a").get("href").replace("..", "")
            ...
            if response.status_code == 200:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                doi_match = re.search(r'DOI:\s*(10\.\d{4,9}/[-._;()/:A-Z0-9]+)', page_ranges_doi_text,
                                                      re.IGNORECASE)
                                ...
                                if date_volume_issue_info:
                                    ...
                                    if date_volume_issue_match:
                                        day = date_volume_issue_match.group(
                                            1) if date_volume_issue_match else "Date not found"
                                        month = date_volume_issue_match.group(
                                            2) if date_volume_issue_match else "Month not found"
                                        year = date_volume_issue_match.group(
                                            3) if date_volume_issue_match else "Year not found"
                                        volume = date_volume_issue_match.group(
                                            4) if date_volume_issue_match else "Volume not found"
                                        issue = date_volume_issue_match.group(
                                            5) if date_volume_issue_match else "Issue not found"
                                    ...
                                    for link in pdf_links:
                                        ...
                                        if pdf_id_match:
                                            ...
                                            base_pdf_url = "https://www.ryhxgy.cn/EN/article/downloadArticleFile.do?attachType=PDF&id="
                                            pdf_link = base_pdf_url + str(pdf_id)

                                            check_value, tpa_id = common_function.check_duplicate(doi, article_title,
                                                                                                  source_id, volume, issue)
                ...
                try:
                    try:
                        response_en = get_response_with_retry(url, headers=headers)
