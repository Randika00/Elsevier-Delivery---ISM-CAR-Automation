...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://journal.nodgo.org"
            toc_url = "https://journal.nodgo.org/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_537.ini")
            ...
            issues = soup.find("div", id="issues").find_all("div", id="issue")

            latest_volume = 0
            latest_issue_number = 0
            latest_year = 0
            latest_link = ""

            for issue in issues:
                issue_link_tag = issue.find("h4").find("a")
                issue_title = issue_link_tag.text
                issue_link = issue_link_tag['href']

                match = re.search(r"Vol (\d+), No (\d+) \((\d{4})\)", issue_title)
                volume, issue, year = "", "", ""
                if match:
                    volume = int(match.group(1)) if match else ""
                    issue = int(match.group(2)) if match else ""
                    year = int(match.group(3)) if match else ""

                    if (volume > latest_volume) or (volume == latest_volume and issue > latest_issue_number):
                        latest_volume = volume
                        latest_issue_number = issue
                        latest_year = year
                        latest_link = issue_link
                        link_id = latest_link.split('/')[-2]

            current_link = f"https://journal.nodgo.org/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fview%2F{link_id}%2FshowToc"
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", id="content").findAll("div", class_="issueArticle flex")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            pages_tag = single_element.find("div", class_="date-row").find("span", class_="pages grey")
                            page_range = ""
                            if pages_tag:
                                page_range = pages_tag.text.strip() if pages_tag else ""

                            last_page_text = single_element.find("div", class_="links").find("span", class_="galleys")
                            last_page_link = ""
                            if last_page_text:
                                last_page_link = last_page_text.find("a", class_="file").get('href') if last_page_text else ""
                            else:
                                print("Last Page Link tag not found.")
                            ...
                            if last_response.status_code == 200:
                                ...
                                url_text = last_soup.find("div", id="pdfDownloadLinkContainer")
                                pdf_link = ""
                                if url_text:
                                    pdf_link = url_text.find("a", class_="action pdf", id="pdfDownloadLink").get("href") if url_text else ""
                                else:
                                    print("Could not extract pdf_link details.")

                                article_response = get_response_with_retry(article_link, headers=headers)
                                if article_response.status_code == 200:
                                    article_soup = BeautifulSoup(article_response.text, 'html.parser')

                                    doi_tag = article_soup.find("div", id="content").find("p")
                                    doi = ""
                                    try:
                                        if doi_tag:
                                            doi = doi_tag.find("a", id="pub-id::doi").get('href').replace("https://doi.org/", "")
                                        else:
                                            print("DOI tag not found in the content.")
                                    except AttributeError:
                                        print("DOI not found for this article.")

                                    check_value, tpa_id = common_function.check_duplicate(doi, article_title, source_id, latest_volume, latest_issue_number)
                                    ...
                                    if Check_duplicate.lower() == "true" and check_value:
                                        ...
                                        data.append(
                                            {"Title": article_title, "DOI": doi, "Publisher Item Type": "",
                                             "ItemID": "",
                                             "Identifier": "",
                                             "Volume": latest_volume, "Issue": latest_issue_number, "Supplement": "", "Part": "",
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('/en_US?', '/ru_RU?')
                        ...
                        chinese_heading = "<h1>Table of Contents - Russian Version</h1>\n"
                        chinese_content = soup_cn.prettify()

                    except Exception as e:
                        error_message = f"Error processing Russian version of URL {toc_url}: {str(e)}"
                        error_list.append(error_message)
                        print(error_message)
                        chinese_heading = "<h1>Error loading Russian version</h1>\n"
