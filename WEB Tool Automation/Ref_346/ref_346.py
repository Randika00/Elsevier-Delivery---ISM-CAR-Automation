...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://journal.equinoxpub.com/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_346.ini")
            ...
            current_link_tag = soup.find("ul", class_="pkp_navigation_primary pkp_nav_list", id="navigationPrimary").find("a", string=lambda text: text and "current" in text.lower())

            current_link = current_link_tag['href'] if current_link_tag else ""
            if not current_link:
                error_list.append(f"Failed to find 'current' link for URL: {url}")
                continue
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if article_links:
                                ...
                                volume_issue_year_info = soup.find("h1")
                                volume, issue, year = "", "", ""
                                if volume_issue_year_info:
                                    volume_issue_year_text = volume_issue_year_info.text.strip()

                                    match = re.match(r"Vol\.\s*(\d+)\s*No\.\s*([0-9\-]+)\s*\((\d{4})\)", volume_issue_year_text)

                                    if match:
                                        volume, issue, year = match.groups() if match else ""

                                    else:
                                        match = re.match(r"Vol\.\s*(\d+)\s*No\.\s*([0-9\-]+)", volume_issue_year_text)
                                        if match:
                                            volume, issue = match.groups() if match else ""

                                month_text = soup.find("div", class_="published").find("span", class_="value")
                                month, year_from_month = "", ""
                                if month_text:
                                    published_date_text = month_text.text.strip()

                                    match = re.match(r"(\d{4})-(\d{2})-(\d{2})", published_date_text)
                                    if match:
                                        date_str = match.group(1)
                                        date_obj = datetime.strptime(published_date_text, '%Y-%m-%d')
                                        month_number = date_obj.month
                                        month_name = date_obj.strftime('%B')
                                        month = month_name
                                        year_from_month = date_obj.year
                                    else:
                                        year_from_month = published_date_text.strip()
                                        month = ""
                                if not year:
                                    year = year_from_month if year_from_month else ""

                                last_page_text = single_element.find("ul", class_="galleys_links")
                                last_page_link = ""
                                if last_page_text:
                                    link = last_page_text.find("a", class_="obj_galley_link pdf")

                                    if link and 'restricted' not in link.get('class', []):
                                        last_page_link = link.get('href') if last_page_text else ""
