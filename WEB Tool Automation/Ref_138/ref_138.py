...
try:
    url = "https://journals.innovareacademics.in/index.php/ajpcr"
    ...
    ini_path = os.path.join(os.getcwd(), "Ref_138.ini")
    ...
    current_link = soup.find("ul", class_="pkp_navigation_primary pkp_nav_list").findAll("li")[2].find("a").get("href")
    print(current_link)
            ...
            try:
                ...
                if article_link_tag:
                    ...
                    for page_range_tag, pdf_url_tag, doi_tag in zip(page_ranges, pdf_url_tags, doi_tags):
                        ...
                        pdf_url, doi = "", ""
                        if 'PDF' or 'PFA' in pdf_url_tag.text:
                            last_page_link = pdf_url_tag.find("a", class_="obj_galley_link pdf").get("href")
                            doi_tag = doi_tag.find("a", class_="doi_link")
                            ...
                            if last_page_link:
                                ...
                                pdf_url_text = last_soup.find("header", class_="header_view").find("a", class_="download")
                                if pdf_url_text:
                                    pdf_url = pdf_url_text.get("href")

                            vol_issue_info = soup.find("h1").text.strip()
                            match = re.match(r"Vol (\d+) Issue (\d+) (\w+) (\d+)", vol_issue_info)
