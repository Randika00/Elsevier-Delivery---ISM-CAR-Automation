...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://vovr.elpub.ru/jour"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_1565.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", id="navbar").find("li", id="current").find("a").get("href")
            current_link = link.replace('/issue/current', '/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fcurrent')
