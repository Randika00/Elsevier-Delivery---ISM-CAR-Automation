                    ...
                    if article_title_div:
                        ...
                        if year_volume_issue_info:
                            ...
                            if article_response.status_code == 200:
                                ...
                                pdf_url_text = article_soup.find("div", class_="journal-panel text-center")
                                pdf_link = ""
                                if pdf_url_text:
                                    pdf_anchor = pdf_url_text.find("a", class_="download btn-small")
                                    if pdf_anchor:
                                        pdf_link = pdf_anchor.get("href", "") if pdf_url_text else ""
                                        print(pdf_link)
                                    else:
                                        print("No PDF link available")
                                else:
                                    print("No PDF section found")

                                # pdf_link = f"https://engstroy.spbstu.ru/userfiles/files/{year}/{volume}{(issue)}/{article_id}.pdf"
                                # if volume and issue:
                                #     formatted_volume_issue = f"{volume}({issue})"
                                # else:
                                #     formatted_volume_issue = volume
                                #
                                # # Construct the correct PDF link
                                # pdf_link = f"https://engstroy.spbstu.ru/userfiles/files/{year}/{formatted_volume_issue}/{article_id}.pdf"
                                # print(pdf_link)
