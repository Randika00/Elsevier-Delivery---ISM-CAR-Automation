...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.epet-info.com/"
            toc_url = "https://www.epet-info.com/dlgcjsen/home"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_3676.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", class_="index_info_r1").findAll("p")[1].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                if script_tag:
                    match = re.search(r"window\.location\.href='(.*?)'", script_tag.text)
                    if match:
                        redirect_url = urllib.parse.urljoin(base_url, match.group(1))
                        # print(f"Redirecting to {redirect_url}")
                        response = get_response_with_retry(redirect_url, headers=headers)
                        if response.status_code == 200:
                            html_content = response.text
                            soup = BeautifulSoup(html_content, "html.parser")
                        else:
                            print(f"Failed to retrieve the redirected page: {response.status_code}")
                    else:
                        print("No redirect URL found in the script tag.")

                div_element = soup.find("div", class_="article_list").findAll("li", class_="article_line")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="article_title")
                        if article_title_div:
                            ...
                            page_range, doi = "", ""
                            if pages_doi_tag:
                                page_range = re.search(r'\d+-\d+', pages_doi_tag.text.strip())
                                doi_tag = pages_doi_tag.find("a", href=True)
                                doi = doi_tag.text.strip() if doi_tag else ""

                                if page_range:
                                    page_range = page_range.group() if pages_doi_tag else ""

                            onclick_text = single_element.find("div", class_="article_toolbar")
                            pdf_link = ""
                            if onclick_text:
                                pdf_link_tag = onclick_text.find("a", class_="btn_pdf").get('href')
                                pdf_link = urllib.parse.urljoin(base_url, pdf_link_tag)

                            volume_issue_year_info = soup.find("ul", class_="article_issue_fl")
                            volume, issue, year = "", "", ""
                            if volume_issue_year_info:
                                match = re.search(r"Volume (\d+),Issue (\d+),(\d+)", volume_issue_year_info.text)
                                if match:
                                    volume = match.group(1) if volume_issue_year_info else ""
                                    issue = match.group(2) if volume_issue_year_info else ""
                                    year = match.group(3) if volume_issue_year_info else ""
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('/dlgcjsen', '/dlgcjs')
