...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://revistas.cesgranrio.org.br/index.php"
            toc_url = "https://revistas.cesgranrio.org.br/index.php/metaavaliacao/user/setLocale/en_US?source=%2Findex.php%2Fmetaavaliacao%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_2438.ini")
            ...
            issue_header = soup.find("div", id="issues").find("h4").find("a")

            link_text = issue_header['href']
            link_id = link_text.split('/')[-1]

            current_link = f"https://revistas.cesgranrio.org.br/index.php/metaavaliacao/user/setLocale/en_US?source=%2Findex.php%2Fmetaavaliacao%2Fissue%2Fview%2F{link_id}%2FshowToc"

            match = re.search(r'Vol (\d+), No (\d+) \((\d{4})\)', issue_header.text)
            volume, issue, year = "", "", ""
            if match:
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", id="content").findAll("table", class_="tocArticle")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="tocTitle")
                        if article_title_div:
                            article_title = article_title_div.find("a").text.strip()
                            article_link = article_title_div.find("a").get('href')

                            pages_tag = single_element.find("td", class_="tocArticleGalleysPages").find("div", class_="tocPages")
                            page_range = ""
                            if pages_tag:
                                page_range = pages_tag.text.strip() if pages_tag else ""

                            last_page_text = single_element.find("td", class_="tocArticleGalleysPages").find("div", class_="tocGalleys")
                            last_page_link = ""
                            if last_page_text:
                            ...
                            if last_response.status_code == 200:
                                ...
                                if article_response.status_code == 200:
                                    ...
                                    doi_tag = article_soup.find("a", id="pub-id::doi")
                                    doi = ""
                                    if doi_tag:
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('/en_US?', '/es_ES?')
                        ...
                        chinese_heading = "<h1>Table of Contents - Spanish Version</h1>\n"
                        chinese_content = soup_cn.prettify()

                    except Exception as e:
                        error_message = f"Error processing Spanish version of URL {toc_url}: {str(e)}"
                        error_list.append(error_message)
                        print(error_message)
                        chinese_heading = "<h1>Error loading Spanish version</h1>\n"
