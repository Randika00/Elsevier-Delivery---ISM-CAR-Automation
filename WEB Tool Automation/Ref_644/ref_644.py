...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://journal11.magtechjournal.com/Jwk_zgyxzz/CN/home"
            toc_url = "http://journal11.magtechjournal.com/Jwk_zgyxzz/EN/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_644.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", id="myTabs", class_="nav nav-tabs aside home-biaoqiannav").findAll("li")[2].find("a").get("href")
            tag_link = base_url + link
            url_en = tag_link.replace('/CN', '/EN')
            current_link = url_en.replace('/home#gkll', '/current')
            ...
            if response.status_code == 200:
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="article-l article-w")
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                date_volume_issue_info = soup.find("span", class_="n-j-q")
                                volume, issue, month, year = "", "", "", ""
                                if date_volume_issue_info:
                                    info_text = date_volume_issue_info.get_text(strip=True)

                                    volume_issue_pattern = re.search(r"(\d{4}) Volume (\d+) Issue (\d+)", info_text)
                                    published_pattern = re.search(r"Published:\s(\d{1,2})\s(\w+)\s(\d{4})", info_text)

                                    if volume_issue_pattern:
                                        year = volume_issue_pattern.group(1) if volume_issue_pattern else ""
                                        volume = volume_issue_pattern.group(2) if volume_issue_pattern else ""
                                        issue = volume_issue_pattern.group(3) if volume_issue_pattern else ""

                                    if published_pattern:
                                        day = published_pattern.group(1) if published_pattern else ""
                                        month = published_pattern.group(2) if published_pattern else ""
                                        published_year = published_pattern.group(3) if published_pattern else ""

                                    token = get_token("http://journal11.magtechjournal.com/Jwk_zgyxzz/EN/home")
                                    pdf_link = f"http://journal11.magtechjournal.com/Jwk_zgyxzz/EN/PDF/{doi}?token={token}"
