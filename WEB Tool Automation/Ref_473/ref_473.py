...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://actamedicaphilippina.upm.edu.ph"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_473.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", id="navigationPrimary", class_="pkp_navigation_primary pkp_nav_list").findAll("li")[0].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("ul", class_="cmp_article_list articles").findAll("div", class_="obj_article_summary")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            volume_issue_year_info = soup.find("li", class_="current")
                            volume, issue, year = "", "", ""
                            if volume_issue_year_info:
                                issue_text = volume_issue_year_info.find("h1").get_text(strip=True)

                                match = re.search(r'Vol (\d+) No (\d+) \((\d+)\)', issue_text)
                                if match:
                                    volume = match.group(1) if issue_text else ""
                                    issue = match.group(2) if issue_text else ""
                                    year = match.group(3) if issue_text else ""
                                else:
                                    print("Volume, issue, and year not found in the expected format.")

                            month_text = soup.find("div", class_="published")
                            month = ""
                            if month_text:
                                date_value = month_text.find("span", class_="value").get_text(strip=True)
                                parsed_date = datetime.strptime(date_value, '%Y-%m-%d') if month_text else ""
                                month = parsed_date.strftime('%B')

                                last_link_text = single_element.find("ul", class_="galleys_links")
                                last_page_link = ""
                                if last_link_text:
                                    last_page_link = last_link_text.find("a", class_="obj_galley_link pdf").get('href') if last_link_text else ""
                                    ...
                                    if pdf_url_info:
                                        ...
                                        if article_response.status_code == 200:
                                            ...
                                            doi_teg = article_soup.find("div", class_="item doi")
                                            doi = ""
                                            if doi_teg:
                                                doi = doi_teg.find("span", class_="value").find("a").get('href').replace("https://doi.org/", "") if doi_teg else ""
