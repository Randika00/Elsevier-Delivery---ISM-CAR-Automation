...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://zwxb.chinacrops.org/EN"
            toc_url = "https://zwxb.chinacrops.org/EN/0496-3490/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_569.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").findAll("ul", class_="dropdown-menu")[1].find("a", string="Current Issue").get("href")
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                page_range_match = re.search(r"\d{4}-\d{4}", page_ranges_doi_text)
                                if page_range_match:
                                    page_range = page_range_match.group(0)

                                doi_match = re.search(r"doi:\S+", page_ranges_doi_text)
                                if doi_match:
                                    doi = doi_match.group(0).replace("doi:", "")
                                ...
                                if date_volume_issue_info:
                                    ...
                                    for link in pdf_links:
                                        ...
                                        if pdf_id_match:
                                            ...
                                            base_pdf_url = "https://zwxb.chinacrops.org/EN/article/downloadArticleFile.do?attachType=PDF&id="
