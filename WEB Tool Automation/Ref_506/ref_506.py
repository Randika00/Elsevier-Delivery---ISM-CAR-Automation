...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://jpad.com.pk/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_506.ini")
                        ...
                        if article_title_div:
                            ...
                            if article_links:
                                ...
                                if volume_issue_year_info:
                                    ...
                                    match = re.search(r"Vol\. (\d+)\s+No\. (\d+)\s+\((\d{4})\):\s+([A-Za-z]+)", volume_issue_year_text)
                                    if match:
                                    ...
                                    print("No volume, issue, year, month metadata found")

                                last_page_text = single_element.find("ul", class_="galleys_links")
                                last_page_link = ""
                                if last_page_text:
                                    last_page_link_element = last_page_text.find("a", class_="obj_galley_link pdf")
                                    if last_page_link_element:
                                        ...
                                        if last_response.status_code == 200:
                                            ...
                                            pdf_link = ""
                                            if pdf_url_info:
                                                pdf_link_element = pdf_url_info.find("a", class_="download")
                                                if pdf_link_element:
                                                    pdf_link = pdf_link_element.get('href')
                                                else:
                                                    print("No PDF link found for this article, skipping.")
                                    else:
                                        print("No 'last_page_link' found, skipping this article.")
                                ...
                                if Check_duplicate.lower() == "true" and check_value:
                                    ...
                                    if pdf_link in downloaded_pdfs:
                                        # print(f"PDF already downloaded: {pdf_link}, skipping.")
                                        continue
