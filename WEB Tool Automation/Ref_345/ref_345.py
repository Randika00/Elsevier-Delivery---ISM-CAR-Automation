...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://journals.riverpublishers.com"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_345.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("div", class_="obj_issue_summary").find("h2").find("a", class_="title").get("href")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            year_volume_issue_info = soup.find("h1")
                            year, volume, issue = "", "", ""
                            if year_volume_issue_info:
                                h1_text = year_volume_issue_info.text.strip()
                                match = re.search(r"(\d{4}):\s*Vol\s*(\d+)\s*Iss\s*(\d+)", h1_text)
                                if match:
                                    year = match.group(1) if h1_text else ""
                                    volume = match.group(2) if h1_text else ""
                                    issue = match.group(3) if h1_text else ""

                            pdf_link_text = single_element.find("ul", class_="galleys_links")
                            last_page_link = ""
                            if pages_text:
                                last_page_link = pdf_link_text.find("a", class_={"obj_galley_link pdf restricted", "obj_galley_link pdf"}).get('href')
                                ...
                                if article_response.status_code == 200:
                                    ...
                                    doi = ""
                                    if doi_tag:
                                        doi = doi_tag.find("span", class_="value").text.strip().replace("https://doi.org/", "") if doi_tag else ""
