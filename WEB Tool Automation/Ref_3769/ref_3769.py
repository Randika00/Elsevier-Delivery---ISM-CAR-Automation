...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.txxb.com.cn/EN"
            toc_url = "http://www.txxb.com.cn/EN/2095-302X/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_3769.ini")
            ...
            if response.status_code == 200:
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_divs = single_element.find_all("td", align="left")

                        for article_title_div in article_title_divs:
                            article_title_tag = article_title_div.find("a", class_="txt_biaoti")
                            if article_title_tag:
                                article_title = article_title_tag.text.strip()
                                article_link = article_title_tag.get('href')

                                doi_tags = single_element.find("span", class_="abs_njq")
                                doi = ""
                                if doi_tags:
                                    doi_text = doi_tags.text.strip()
                                ...
                                if date_volume_issue_info:
                                    ...
                                    match = re.search(r'(\d{1,2})\s(\w+)\s(\d{4}),\sVolume\s(\d+)\sIssue\s(\d+)', date_volume_issue_text)
                                    if match:
                                        day = match.group(1) if date_volume_issue_text else ""
                                        month = match.group(2) if date_volume_issue_text else ""
                                        year = match.group(3) if date_volume_issue_text else ""
                                        volume = match.group(4) if date_volume_issue_text else ""
                                        issue = match.group(5) if date_volume_issue_text else ""

                                article_response = get_response_with_retry(article_link, headers=headers)
                                if article_response.status_code == 200:
                                    article_soup = BeautifulSoup(article_response.text, 'html.parser')

                                    pages_info = article_soup.find("div", class_="col-md-12").find("span")
                                    page_range = ""
                                    if pages_info:
                                        text = pages_info.get_text(strip=True)
                                    ...
                                    onclick_attr = single_element.find("div", class_="links1").find_all("a")
                                    id_value = ""
                                    if onclick_attr:
                                        for tag in onclick_attr:
                                            onclick_text = tag.get("onclick", "")
                                            match = re.search(r"lsdy1\('[^']+','(\d+)'", onclick_text)
                                            if match:
                                                id_value = match.group(1) if onclick_text else ""

                                                pdf_url = f"http://www.txxb.com.cn/EN/article/downloadArticleFile.do?attachType=PDF&id={id_value}"
