...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://ebcj.mums.ac.ir"
            article_tag = "https://ebcj.mums.ac.ir/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_2669.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").find("ul", class_="dropdown-menu").find("a", string="Current Issue").get("href").replace(".", "")
            ...
            if response.status_code == 200:
                ...
                articles = soup.find("div", class_="page-header margin-top-3", id="dv_ar_count")
                if articles:
                    all_articles = articles.find("span", class_="badge badge-light").text.strip()
                    ...
                    for single_element in div_element:
                        ...
                        try:
                            ...
                            if article_title_div:
                                ...
                                pages_text = single_element.find("p", class_="margin-bottom-3")
                                page_range = ""
                                if pages_text:
                                    page_range = pages_text.find("span").text.strip() if pages_text else None
                                else:
                                ...
                                doi = ""
                                if doi_tag:
                                    doi = doi_tag.find("span", dir="ltr").text.strip().replace("https://doi.org/", "") if doi_tag else ""
                                else:
                                    print("DOI not found.")

                                volume_issue_date_info = soup.find("div", class_="weight-200 nomargin-top")
                                volume, issue, month, year = "", "", "", ""
                                if volume_issue_date_info:
                                    volume_issue_date_text = volume_issue_date_info.find("span").text.strip()
                                    pattern = r"Volume (\d+), Issue (\d+).*, (\w+)\s(\d{4})"
                                    match = re.search(pattern, volume_issue_date_text)
                                    if match:
                                ...
                                if article_response.status_code == 200:
                                    ...
                                    xml_text = article_soup.find("ul", class_="list-group list-group-bordered list-group-noicon nomargin").find("li", class_="list-group-item")
                                    xml_link = ""
                                    if xml_text:
                                        xml_info = xml_text.find("a", class_="tag_a pdf_link").get('href').replace("./", "/") if xml_text else ""
                                        xml_link = urllib.parse.urljoin(base_url, xml_info)
                                    else:
                                        print("No xml link found")

                                    pdf_url_info = article_soup.find("ul", class_="list-group list-group-bordered list-group-noicon nomargin").findAll("li", class_="list-group-item")[1].find("a", class_="tag_a pdf_link").get('href').replace("./", "/")
                                    pdf_link = urllib.parse.urljoin(base_url, pdf_url_info)
                                    ...
                                    if Check_duplicate.lower() == "true" and check_value:
                                        ...
                                        data.append(
                                            {"Title": article_title, "DOI": doi, "Publisher Item Type": "",
                                             ...
                                             "XML File Name": f"{pdf_count}.xml", "user_id": user_id
                                             })
