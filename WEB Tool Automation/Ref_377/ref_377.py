...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://bulletinofcas.researchcommons.org"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_377.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("div", id="homepage-intro").findAll("p")[1].find("a").get("href")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_link_tag:
                            ...
                            volume_issue_info = soup.find("div", id="alpha").find("h1").text.strip()
                            match = re.search(r'Volume (\d+), Issue (\d+)\s*\((\d{4})\)', volume_issue_info)
                            ...
                            if article_response.status_code == 200:
                                ...
                                pages_text = article_soup.find("div", id="alpha").findAll("div", class_="element", id={"fpage", "lpage"})
                                page_range = ""
                                if pages_text:
                                    first_page = None
                                    last_page = None
                                    ...
                                    for page in pages_text:
                                        if page.get("id") == "fpage":
                                            first_page = page.find("p").text.strip() if pages_text else ""
                                        elif page.get("id") == "lpage":
                                            last_page = page.find("p").text.strip() if pages_text else ""

                                    if first_page and last_page:
                                        page_range = f"{first_page}-{last_page}"
                                    else:
                                        print("Page range information not found.")

                                pdf_url_tag = article_soup.find("div", class_="aside download-button")
                                pdf_link = ""
                                if pdf_url_tag:
                                    pdf_link = pdf_url_tag.find("a", class_="btn", id="pdf").get("href") if pdf_url_tag else ""
