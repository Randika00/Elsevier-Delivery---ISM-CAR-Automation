...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.zgmfskin.com/EN/1009-1157/home.shtml"
            toc_url = "http://www.zgmfskin.com/EN/1009-1157/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_454.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", id="myTabs", class_="nav nav-tabs").find("li", class_="j-title pull-left active").find("a").get("href")
            home_link = base_url + link
            current_link = home_link.replace('/home.shtml#dangqi', '/current.shtml')
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="article-box paddingtb20").findAll("dl", class_="row")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("dd", class_="col-md-12 col-sm-12 col-xs-12")
                        if article_title_div:
                            article_title = article_title_div.find("h4").find("a", class_="text-title").text.strip()
                            article_link = article_title_div.find("h4").find("a", class_="text-title").get('href')

                            month_tag = single_element.find("p", class_="doi")
                            month = ""
                            if month_tag:
                                text_info = month_tag.get_text(strip=True)
                                match = re.match(r"(\d{4}), Vol\.(\d+)\((\d+)\): (\d+–\d+)", text_info)
                                if match:
                                    month_number = match.group(3) if text_info else ""
                                    month = month_map.get(month_number, "Unknown Month")

                            onclick_attr = single_element.find_all("span", class_="ico pdf")
                            pdf_link_id, pdf_link = "", ""
                            if onclick_attr:
                                for pdf_element in onclick_attr:
                                    onclick_value = pdf_element.find('a').get('onclick')
                                    pdf_link_id = re.search(r"lsdy1\('PDF','(\d+)'", onclick_value).group(1)

                                    base_pdf_url = "http://www.zgmfskin.com/EN/article/downloadArticleFile.do?attachType=PDF&id="
                                    pdf_link = base_pdf_url + str(pdf_link_id)

                                year_volume_issue_pages_doi_info = single_element.find("p", class_="doi")
                                year, volume, issue, page_range, doi = "", "", "", "", ""
                                if year_volume_issue_pages_doi_info:
                                    text_info = year_volume_issue_pages_doi_info.get_text(strip=True)

                                    match = re.match(r"(\d{4}), Vol\.(\d+)\((\d+)\): (\d+–\d+)", text_info)
                                    if match:
                                        year = match.group(1) if text_info else ""
                                        volume = match.group(2) if text_info else ""
                                        issue = match.group(3) if text_info else ""
                                        page_range = match.group(4) if text_info else ""

                                    doi = year_volume_issue_pages_doi_info.find("a").get("href").replace("https://doi.org/", "") if year_volume_issue_pages_doi_info else ""
