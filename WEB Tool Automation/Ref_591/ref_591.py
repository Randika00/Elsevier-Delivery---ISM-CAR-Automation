...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://english.cjebm.com"
            toc_url = "https://english.cjebm.com/journal/zgxzyxzz"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_591.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").find("li", type={"home"}).find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("ul", id="currentList", class_="article-list").findAll("li", class_="article-list-cite")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            pages_year_info = single_element.find("span", class_="page")
                            page_range, year, volume, issue = "", "", "", ""
                            if pages_year_info:
                                info_text = pages_year_info.text.strip()

                                match = re.match(r"(\d{4}),\s*(\d+)\((\d+)\):\s*([\d\-]+)", info_text)
                                if match:
                                    year = match.group(1)
                            ...
                            id_value = single_element.find("span", class_="ctrl").find("a", class_="citedby-font")
                            id_no = ""
                            if id_value:
                                id_no = id_value.get("id")

                            pdf_link = f"https://english.cjebm.com/indexAction!downloadPdfByOss.action?id=NjE3Mzc={id_no}"

                            doi_text = single_element.find("span", class_="ctrl").find("a", class_="citedby-font")
                            doi = ""
                            if doi_text:
                                doi = doi_text.text.strip().replace("/article/", "") if id_value else ""

                            # article_response = get_response_with_retry(article_link, headers=headers)
                            # if article_response.status_code == 200:
                            #     article_soup = BeautifulSoup(article_response.text, 'html.parser')
                            #
                            #     doi_tag = article_soup.find("div", class_="main-info form-horizontal")
                            #     print(doi_tag)
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('https://english.cjebm.com', 'https://www.cjebm.com')
