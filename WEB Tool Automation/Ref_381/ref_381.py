...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.irla.cn/indexen.htm"
            toc_url = "http://www.irla.cn/indexen.htm#current"
            Base_URL = "http://www.irla.cn"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_381.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", id="myTab", class_="tab-ul tab-ul-index clearfix").findAll("li")[1].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", id="currentIssue", class_="articleListRenderCount").findAll("div", class_="article-list")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            article_info = single_element.find("div", class_="article-list-time")
                            year, volume, issue, identifier, doi = "", "", "", "", ""
                            if article_info:
                                year_volume_issue_text = article_info.find_all("font")[0].text.strip()
                                doi = article_info.find_all("font")[1].find("a").text.strip() if article_info else ""

                                year_volume_issue_match = re.search(r'(\d{4}), (\d+)\((\d+)\): (\d+)', year_volume_issue_text)

                                if year_volume_issue_match:
                                    year = year_volume_issue_match.group(1) if year_volume_issue_match else ""
                                    volume = year_volume_issue_match.group(2) if year_volume_issue_match else ""
                                    issue = year_volume_issue_match.group(3) if year_volume_issue_match else ""
                                    identifier = year_volume_issue_match.group(4) if year_volume_issue_match else ""
                                ...
                                pdf_url = f"http://www.irla.cn/article/exportPdf?id={id_values_string}&language=en"
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('/indexen', '/index')
