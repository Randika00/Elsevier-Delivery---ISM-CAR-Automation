...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://mongoloved.kigiran.com"
            toc_url = "https://mongoloved.kigiran.com/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_502.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", id="issues").find("div", id="issue").find("a")["href"]
            link_id = link.split('/')[-1]
            current_link = f"https://mongoloved.kigiran.com/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fview%2F{link_id}"

            issue_div = BeautifulSoup(response.content, "html.parser").find("div", id="issues").find("div", id="issue")
            issue_text = issue_div.find("h4").text.strip()

            match = re.search(r"Vol (\d+), No (\d+) \((\d{4})\)", issue_text)
            volume, issue, year = "", "", ""
            if match:
                volume = match.group(1) if issue_text else ""
                issue = match.group(2) if issue_text else ""
                year = match.group(3) if issue_text else ""
            else:
                print("Error: Could not extract volume, issue, and year.")
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if article_response.status_code == 200:
                                ...
                                doi = ""
                                if doi_tag:
                                    doi = doi_tag.find("a", id="pub-id::doi").get('href').replace("https://doi.org/", "") if doi_tag else ""
