...
try:
    ...
    for line in url_details:
        try:
            ...
            ini_path = os.path.join(os.getcwd(), "Ref_519.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("a", href=True, string=lambda s: "Volume" in s if s else False)
            current_link, volume, issue = "", "", ""
            if link:
                link_tag = link['href']
                current_link = base_url + link_tag

                volume = link_tag.split('vol=')[1].split('&')[0]
                issue = link_tag.split('no=')[1]
            else:
                print("Target link not found.")

            toc_url = f"https://www.kjorl.org/current/index.php?vol={volume}&no={issue}&lang=en"
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("table", attrs={"border": "0", "width": "100%", "cellpadding": "0", "cellspacing": "0", "align": "center"}).findAll("tr")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_td = single_element.find("td", style="padding-left:10px;padding-right:27px; padding-top:10px;border:1px solid #ffffff;padding-bottom:10px;font-weight:bold;")
                        if article_title_td:
                            article_title_tag = article_title_td.find("a")
                            if article_title_tag:
                                article_title = article_title_tag.text.strip()
                                article_links = article_title_tag['href']
                                ...
                                if article_title not in printed_titles:
                                    ...
                                    pages_doi_tag = single_element.find("div", class_="article_txt2", style="padding-top:2px;padding-left:0px")
                                    page_range, doi = "", ""
                                    if pages_doi_tag:
                                        pages_text = pages_doi_tag.get_text(strip=True)

                                        page_range_match = re.search(r'(\d+-\d+)', pages_text)
                                        doi_match = re.search(r'https?://doi\.org/([^\s]+)', pages_text)

                                        if page_range_match:
                                            page_range = page_range_match.group(0)

                                        if doi_match:
                                            doi = doi_match.group(0).replace("https://doi.org/", "") if doi_match else ""

                                    pdf_text = single_element.find("td", class_="article_txt2", style="padding-top:5px;padding-left:0px")
                                    pdf_link = ""
                                    if pdf_text:
                                        match = re.search(r'journal_download\("pdf", "\d+", "([^"]+)"\)', str(pdf_text))
                                        id_value = ""
                                        if match:
                                            id_value = match.group(1) if pdf_text else ""

                                            base_pdf_url = "https://www.kjorl.org/upload/pdf/"
                                            pdf_link = base_pdf_url + id_value

                                    month_year_nfo = soup.find("td", style="padding-left:10px;color:#01944D;font-weight:bold")
                                    month, year = "", ""
                                    if month_year_nfo:
                                        match = re.search(r"([A-Za-z]+),\s*(\d{4})", month_year_nfo.get_text(strip=True))
                                        if match:
                                            month = match.group(1) if month_year_nfo else ""
                                            year = match.group(2) if month_year_nfo else ""
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('&lang=en', '&lang=ko')
                        ...
                        chinese_heading = "<h1>Table of Contents - Korean Version</h1>\n"
                        chinese_content = soup_cn.prettify()

                    except Exception as e:
                        error_message = f"Error processing Korean version of URL {toc_url}: {str(e)}"
                        error_list.append(error_message)
                        print(error_message)
                        chinese_heading = "<h1>Error loading Korean version</h1>\n"
