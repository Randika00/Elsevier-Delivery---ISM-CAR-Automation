...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.pibb.ac.cn"
            toc_link = "http://www.pibb.ac.cn/pibben/issue/current"
            article_url = "http://www.pibb.ac.cn/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_448.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", class_="n2").find("div", class_="m1")

            if link:
                text = link.get_text()

                match = re.search(r"(\d{4}),\s*(\d+)\((\d+)\)", text)
                year, volume, issue = "", "", ""
                if match:
                ...
                current_link = f"http://www.pibb.ac.cn/pibben/article/issue/{year}_{volume}_{issue}"
                toc_url = f"http://www.pibb.ac.cn/pibben/article/issue/{year}_{volume}_{issue}"
                ...
                if response.status_code == 200:
                    ...
                    for single_element in div_element:
                        ...
                        try:
                            ...
                            if article_title_div:
                                ...
                                token = single_element.find("input", class_="article_issue_fl")
                                id_value = ""
                                if token:
                                    id_value = token.get("value")

                                pages_doi_tag = single_element.find("p", class_="article_position")
                                page_range, doi = "", ""
                                if pages_doi_tag:
                                    page_ranges_doi_text = pages_doi_tag.get_text(strip=True)
                                    match = re.search(r'\((\d+)\):(\d+-\d+)\.DOI:([\S]+?)(?=\s*CSTR:|$)', page_ranges_doi_text)
                                    ...
                                    if match:
                                        ...
                                        print("No pages, DOI data found!")

                                pdf_link_tag = single_element.find("div", class_="article_toolbar")
                                pdf_link = ""
                                if pdf_link_tag:
                                    pdf_links = pdf_link_tag.find("a", class_="btn_pdf").get('href')
                                    pdf_link = urllib.parse.urljoin(article_url, pdf_links)

                                article_response = get_response_with_retry(article_link, headers=headers)
                                if article_response.status_code == 200:
                                    article_soup = BeautifulSoup(article_response.text, 'html.parser')

                                    date_month_text = article_soup.find("div", class_="article_abstract_right m-fl").find("div", class_="p6").find("li", id="BookPublishedTime")
                                    day, month = "", ""
                                    if date_month_text:
                                        date_text = date_month_text.find("span", id="BookPublishedTimeValue")
                                        if date_text:
                                            ...
                                            if match:
                                                ...
                                                day = match.group(2) if match else ""
                    ...
                    try:
                        ...
                        try:
                            url_cn = toc_url.replace('/pibben', '/pibb')
