...
# Function to get response with retry logic
def get_response_with_retry(url, headers, retries=3, backoff_factor=0.3, status_forcelist=(500, 502, 504), verify=False):
...
try:
    # Read the URL details from the file
    try:
    ...
    # Read completed URLs from the file
    try:
    ...
    # Process each URL in the urlDetails.txt file
    for line in url_details:
        try:
            url, source_id = line.strip().split(",")
            base_url = "https://cykx.lzu.edu.cn"
            toc_url = "https://cykx.lzu.edu.cn/en/article/current"
            ...
            ini_path = os.path.join(os.getcwd(), "Ref_3748.ini")
            ...
            # Get the response for the initial URL
            response = get_response_with_retry(url, headers=headers, verify=False)
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav-inner clearfix container").find("li", type="dangqimulu_en").find("a").get("href")
            current_link = base_url + link

            # Get the response for the TOC page
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            pdf_url = f"https://cykx.lzu.edu.cn/article/exportPdf?id={id_values_string}&language=en"
                            ...
                            if Check_duplicate.lower() == "true" and check_value:
                                ...
                                pdf_content = get_response_with_retry(pdf_url, headers=headers, verify=False).content
