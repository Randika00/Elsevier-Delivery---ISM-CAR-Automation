...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://zgnyqx.ieda.org.cn//EN"
            toc_url = "https://zgnyqx.ieda.org.cn/EN/1000-6362/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_521.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", class_="tab_zxqk border border-radius").findAll("li")[1]
            current_link = link.find("a")["href"]
            ...
            if response.status_code == 200:
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="dqml_qbwz")
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                match = re.search(r"(\d+-\d+)\.\s+doi:(10\.\d{4,}/[\w.-]+)", page_ranges_doi_text)
                                if match:
                                    page_range = match.group(1) if page_ranges_doi_text else ""
                                    doi = match.group(2) if page_ranges_doi_text else ""
                            ...
                            token = get_token("https://zgnyqx.ieda.org.cn//")
                            pdf_link = f"https://zgnyqx.ieda.org.cn/EN/PDF/{doi}?token={token}"
