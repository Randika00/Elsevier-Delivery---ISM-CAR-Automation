...
ch_url="https://jme.biam.ac.cn/CN/1001-4381/home.shtml"
...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_773.ini")
        ...
        current_issu_link=soup.find_all("ul",class_="dropdown-menu")[2].find('a', string="Current Issue").get('href')

        date_time_meta=soup.find("span",class_="n-j-q nianjq").get_text(strip=True)
        pattern = r"(?P<year>\d{4}) Volume (?P<volume>\d+) Issue (?P<issue>\d+)Published: (?P<date>\d{1,2}) (?P<month>\w+)"
        ...
        try:
            ...
            if match:
                ...
                print(f"Month: {month}, Date: {date}, Year: {year}, Volume: {volume}, Issue: {issue}")
            else:
                print("No match found.")
        except Exception as e:
            print(f"An error occurred: {e}")
        All_articles =soup.find_all("div",class_="article-l")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title = All_articles[article_index].find("div", class_="j-title-1").find("a").get_text(strip=True)
                try:
                    DOI_element = All_articles[article_index].find("div", class_="j-volumn-doi").find("a",class_="j-doi")
                ...
                page_range_text=All_articles[article_index].find("div", class_="j-volumn-doi").find("span",class_="j-volumn").get_text(strip=True)
                pattern = r":\s*(?P<page_range>\d+-\d+)\."
                ...
                try:
                    ...
                    if match:
                        ...
                        print("Page range not found in the given text.")
                except Exception as e:
                    page_range =""
                    print(f"An error occurred: {e}")
                ...
                print(token)
                try:
                    pdf_link = f"https://jme.biam.ac.cn/EN/PDF/{DOI}?token={token}"
                except Exception as e:
                    print(f"An error occurred: {e}")
                if article_check == 0:
