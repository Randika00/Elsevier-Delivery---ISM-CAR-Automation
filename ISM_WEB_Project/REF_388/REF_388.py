...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_388.ini")
        ...
        date_time_meta=soup.find("div",class_="meta-title").get_text(strip=True)
        pattern = r'Volume\s*(\d+),\s*Issue\s*(\d+).*?(\d{4})'

        # Using regex to search for the pattern
        match = re.search(pattern, date_time_meta)

        # Extracting and displaying results if found
        if match:
        ...
        All_articles =soup.find_all("div",class_="latest-article-block")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://www.pm-research.com" + All_articles[article_index].find("a").get("href")
                Article_title = All_articles[article_index].find("span", class_="field--highwire-content-title").get_text(strip=True)

                soup2=get_soup(Article_link)
                try:
                    doi = soup2.find("div", class_="journal-article-doi").get_text(strip=True).split("DOI:")[-1].strip().strip("'")
                except Exception as e:
                    print(f"Exception: {e}")

                try:
                    page_range_text = soup2.find("div", class_="journal-article-description").get_text(strip=True)
                    page_range_pattern = r'(\d+)\s*-\s*(\d+)'

                    try:
                        page_range_match = re.search(page_range_pattern, page_range_text)
                        if page_range_match:
                            start_page = page_range_match.group(1)
                            end_page = page_range_match.group(2)
                            page_range = f"{start_page}-{end_page}"
                            print(f"Page Range: {page_range}")
                        else:
                            raise ValueError("Page range pattern not found.")
                    except Exception as e:
                        print(f"Exception: {e}")
                except Exception as e:
                    print(f"Exception: {e}")
                ...
                try:
                    pdf_link = f"https://www.pm-research.com{All_articles[article_index].find("a",class_="btn download journal-btn-first").get("href")}"
                except Exception as e:
                    print(f"Exception: {e}")
