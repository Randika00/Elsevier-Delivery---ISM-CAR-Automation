...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_3306.ini")
        ...
        En_Href=soup.find("div",class_="language").find("a").get("data")
        soup2=get_soup(En_Href)

        combine_soup = str(soup) + str(soup2)
        ...
        date_time_meta=soup2.find("div",class_="tocArticle flex").find("div",class_="title").get_text(strip=True)
        pattern = r"No (\d+) \((\d{4})\)"
        ...
        All_articles =soup2.find_all("div",class_="issueArticle flex")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title = All_articles[article_index].find("div", class_="title").find("a").get_text(strip=True)
                page_range=All_articles[article_index].find("div", class_="date-row").find("span",class_="pages grey").get_text(strip=True)
                pdf_link_pre = All_articles[article_index].find("div",class_="links").find("a",class_="file").get("href")
                pdf_link = pdf_link_pre.replace('/view/', '/download/')
                doi=""
