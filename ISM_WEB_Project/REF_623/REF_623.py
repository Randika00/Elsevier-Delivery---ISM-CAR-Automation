...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_623.ini")
        ...
        Ru_link = soup.find("li", class_="active").find_next_sibling("li").find("a").get("href")
        Ru_soup = get_soup(Ru_link)
        ...
        date_time_meta=soup.find_all("h2")[1].get_text(strip=True)
        ...
        All_articles =soup.find("table",class_="spec arhiveItem").find_all("td",class_="one")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                articile_page=get_soup(Article_link)
                Article_title = articile_page.find("article",class_="article").find("h2").get_text(strip=True)
                li_tag=articile_page.find("div",class_="titleAuthors").find_all("li")[2]
                page_range = None

                if li_tag:
                    # Regular expression to extract the page range
                    match = re.search(r"Pages:\s*(\d+-\d+)", li_tag.text)
                    if match:
                        page_range = match.group(1)

                DOI_element = articile_page.find("div",class_="titleAuthors").find_all("li")[-1].find("a")

                if DOI_element:
                    doi = DOI_element.get_text(strip=True).split("doi.org/")[-1]
                else:
                    doi = "DOI not found"
                original_link = articile_page.find("div",class_="downloadPDF").find("a", href=True).get("href")
                if original_link:
                    pdf_link = original_link.replace("/view/", "/download/")
                else:
