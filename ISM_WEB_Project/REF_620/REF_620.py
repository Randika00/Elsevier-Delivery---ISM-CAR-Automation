...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_620.ini")
        ...
        soup=get_soup(url)
        current_issue_href=soup.find("ul",class_="pkp_navigation_primary pkp_nav_list").find_all("a")[1].get("href")
        soup2=get_soup(current_issue_href)

        date_time_meta=soup2.find("h1").get_text(strip=True)
        pattern = r"Vol\.\s*(\d+)\s*No\.\s*(\d+)\s*\((\d{4})\):\s*(\w+)"

        # Dictionary to map season to month
        season_to_month = {
            "Summer": "June",
            "Spring": "March",
            "Fall": "September",
        ...
        if match:
            ...
            month = season_to_month.get(season, "Unknown")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    DOI_element = soup3.find("span", class_="value").find("a")
                ...
                try:
                    pdf_link = All_articles[article_index].find("a", class_="obj_galley_link pdf").get("href")
                except AttributeError:
                    pdf_link = None
                    print("PDF link not found for the specified article.")
