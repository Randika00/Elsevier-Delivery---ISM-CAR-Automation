...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_564.ini")
        ...
        current_issue_tag = soup.find('div', id='content-archiwum').div.find_all('li')
        current_issue_link = "http://www.medycynawet.edu.pl"+[li for li in current_issue_tag if li.find('a')][-2].find('a').get('href')

        current_issue_soup=get_soup(current_issue_link)
        text = current_issue_soup.find("div", id="adres-bib").text.strip()
        ...
        All_articles = current_issue_soup.find_all('div', class_='artykul')
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "http://www.medycynawet.edu.pl"+All_articles[article_index].find("span", class_="summary").find('a').get('href')

                pdf_page_soup=get_soup(Article_link)

                Article_title = pdf_page_soup.find("td", class_="tytul").text.strip()
                page_range_text = pdf_page_soup.find("div", id="adres-bib").text.strip()
                ...
                if match:
                    page_range = match.group(0)
                    print(f"page_range='{page_range}'")
                else:
                    print("No match found")

                doi =  pdf_page_soup.find('div', id='doi').find('a').text.strip().split("doi.org/")[-1]
                print_bordered_message(f"Page Range: {page_range}, DOI: {doi}")

                pdf_link = "http://www.medycynawet.edu.pl"+pdf_page_soup.find("div",id="gopdf").find("a").get('href')
