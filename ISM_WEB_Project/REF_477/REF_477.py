...
while url_index < len(url_list):
    try:
        cn_href ="http://rockmech.whrsm.ac.cn/CN/volumn/home.shtml"
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_477.ini")
        ...
        combine_soup = str(soup) + str(cn_href)
        ...
        target_text = soup.find('span',class_="STYLE5").get_text(strip=True)
        pattern = r'(\d{4}),\s*(\d+)\s*\((\w+)\)'
        ...
        All_articles =soup.find_all("div",class_="noselectrow")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "http://rockmech.whrsm.ac.cn" + All_articles[article_index].find("p", class_="abs_biaoti").find("a").get("href")
                Article_title = All_articles[article_index].find("p", class_="abs_biaoti").find("a").get_text(strip=True)
                page_range_tag = All_articles[article_index].find('strong')
                page_range_tag2 = page_range_tag.find_next_sibling(string=True).strip() if page_range_tag else None
                if page_range_tag2:
                    page_range = page_range_tag2.split(":")[-1].strip()
                doi = All_articles[article_index].find(string="doi:").next.strip()

                pdf_link_tag = All_articles[article_index].find('a', class_='J_WenZhang_U',onclick=lambda value: 'PDF' in value if value else False)
                if pdf_link_tag:
                    pdf_section = pdf_link_tag.parent.get_text(strip=True)
                    onclick_value = pdf_link_tag.get('onclick')
                    pdf_id = onclick_value.split(",")[1].strip().replace(")", "").replace("'", "")
                    pdf_link = f"http://rockmech.whrsm.ac.cn/EN/article/downloadArticleFile.do?attachType=PDF&id={pdf_id}"
                else:
