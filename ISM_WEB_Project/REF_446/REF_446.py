...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_446.ini")
        ...
        current_issue_link = soup.find('ul', class_='dropdown-menu').find('a').get('href')

        current_issue_soup=get_soup(current_issue_link)
        text = current_issue_soup.find("ol", class_="breadcrumb").find("li",class_="active").text.strip()

        pattern = r"Vol\.\s(\d+)\sNo\.\s(\d+)\s\((\d+)\)"
        ...
        All_articles = current_issue_soup.find_all('div', class_='media-body')
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title = All_articles[article_index].find("h3", class_="media-heading").text.strip()
                original_link = All_articles[article_index].find("div", class_="btn-group").find("a",class_="galley-link btn btn-primary pdf").get('href')
                pdf_link = original_link.replace("view", "download")

                pdf_page_soup=get_soup(Article_link)
                try:
                    doi =  pdf_page_soup.find('div', class_='list-group-item doi').find('a').text.strip().split("doi.org/")[-1]
                except Exception as error:
