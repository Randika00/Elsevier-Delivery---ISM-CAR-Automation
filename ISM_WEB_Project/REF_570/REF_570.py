...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_570.ini")
        ...
        RU_href = soup.find('div', class_="ru_RU").find('a').get('href')
        RU_soup = get_soup(RU_href)

        CH_href = soup.find('div', class_="zh_CN").find('a').get('href')
        CH_soup = get_soup(RU_href)

        combine_soup = str(soup) + str(RU_soup)+ str(CH_soup)
        ...
        date_time_meta=soup.find("h1").get_text(strip=True)
        pattern = r"Vol\s*(\d+),\s*No\s*(\d+)\s*\((\d{4})\)"

        # Use re.search to find and extract the groups
        ...
        All_articles =soup.find_all("h3",class_="articleTitle")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                meta_data=soup2.find("div",class_="titleAuthors")
                try:
                    page_range_element = meta_data.find_all('li')[-8]
                    page_range = page_range_element.get_text(strip=True).split(":")[-1].strip() if page_range_element else None
                ...
                try:
                    # Extracting the DOI
                    doi_element = meta_data.find_all('li')[-2]
                    doi = doi_element.get_text(strip=True).split("doi.org/")[-1] if doi_element else None
                except Exception as e:
                    doi = "DOI not found"
                    print(f"Error extracting DOI: {e}")

                try:
                    # Extracting the identifier using 'string' instead of 'text'
                    identifier_element = meta_data.find_all('li')[-1]
                    if identifier_element:
                        match = re.search(r'ID:\s*(\d+)', identifier_element.get_text(strip=True))
                ...
                try:
                    pdf_link1=soup2.find("div",class_="downloadPDF").find("a",class_="file").get("href")
                    pdf_lin1_soup=get_soup(pdf_link1)
                    pdf_link = pdf_lin1_soup.find("a",class_="action pdf").get("href")
                except Exception as e:
                    pdf_link = "pdf_link not found"
                    print(f"Error extracting pdf_link: {e}")
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    if download_successful:
                        ...
                        data.append({
                            ...
                            "Identifier": identifier, "Volume": volume, "Issue": issue, "Supplement": "", "Part": "",
