...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_1458.ini")
        ...
        links = soup.find("a",class_="link--external brz-cp-color7").get("href")
        soup2=get_soup(links)

        current_issue=soup2.find_all("p")[1].find("a").get("href")
        soup3=get_soup(current_issue)

        date_time_meta=soup2.find_all("p")[1].find("a").get_text(strip=True)
        pattern = r"Volume (\d+), Issue (\d+) \((\w+)-(\w+)\), (\d{4})"
        ...
        for td in soup3.find_all('td'):
            # Filter out elements that are just headers (they tend to have a short length or simple text)
            if len(td.get_text(strip=True)) > 50:  # Adjust the length threshold as needed
                All_articles.append(str(td))
        ...
        while article_index < len(All_articles):
            ...
            try:
                article = BeautifulSoup(All_articles[article_index], 'html.parser')

                title_tag = article.find('strong')
                Article_title = title_tag.get_text(strip=True) if title_tag else "No Title"

                # Extracting DOI link
                doi_tag = article.find('a', title="DOI")
                doi = doi_tag['href'].split("doi.org/")[-1] if doi_tag else "No DOI"

                # Extracting PDF link
                pdf_tag = article.find('a', title="PDF")
                pdf_link = pdf_tag['href'] if pdf_tag else "No PDF"
