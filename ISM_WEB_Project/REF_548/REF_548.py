...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_548.ini")
        ...
        soup=get_soup(url)
        current_volume_link="https://journal.uob.edu.bh"+soup.find_all("ul")[6].findAll("a")[-1].get("href")
        soup2=get_soup(current_volume_link)

        volume_text=soup2.find("h1",class_="ds-div-head").get_text(strip=True)
        match = re.search(r'Volume\s+(\d+)', volume_text)

        # Extract the volume if match is found
        volume = match.group(1) if match else None

        print(f"volume = '{volume}'")

        issue_text = soup2.find("span", class_="issuetext").get_text(strip=True)
        match = re.search(r'Issue\s*(\d+)', issue_text)
        issue = match.group(1) if match else None

        print(f'issue = "{issue}"')

        date_text = soup2.find("span", class_="short-description").get_text(strip=True)
        ...
        All_articles =soup2.find_all("div",class_="artifact-description")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://journal.uob.edu.bh" + All_articles[article_index].find("div", class_="artifact-title").find("a").get("href")
                Article_title = All_articles[article_index].find("div", class_="artifact-title").find("a").get_text(strip=True)

                pdf_page_soup=get_soup(Article_link)

                DOI_element = pdf_page_soup.find("div", class_="simple-item-view-other").find("a")
                ...
                pdf_link = f"https://journal.uob.edu.bh{pdf_page_soup.find("a",class_="image-link").get("href")}"
