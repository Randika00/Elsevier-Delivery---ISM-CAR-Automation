...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_1074.ini")
        ...
        current_issue_link="http://kykxxb.cumtb.edu.cn/cn/article/current"
        soup=get_soup(url)
        ...
        date_time_meta=soup.find("title").get_text(strip=True)
        match = re.search(r'(\d{4})\s+Vol\.\s+(\d+)\s+No\.\s+(\d+)', date_time_meta)
        ...
        All_articles =soup.find_all("div",class_="article-list-right")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "http://kykxxb.cumtb.edu.cn" + All_articles[article_index].find("div", class_="article-list-title clearfix").find("a").get("href")
                ...
                pdf_link = f"http://kykxxb.cumtb.edu.cn/article/exportPdf?id={All_articles[article_index].find("div",class_="article-list-zy article-list-btn clear").find('a', onclick=True).get('onclick').split("'")[1]}&language=en"
