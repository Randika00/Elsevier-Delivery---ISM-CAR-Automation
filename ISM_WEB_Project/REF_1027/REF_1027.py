...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_1027.ini")
        ...
        date_time_meta=soup.find_all("h2",class_="main_title")[1].get_text(strip=True)
        ...
        articles=soup.find_all("div",id="main_area")[1]
        All_articles =articles.find_all("div",class_="detail")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find("h1", class_="title_paper").find("a").get("href")
                Article_title = All_articles[article_index].find("h1", class_="title_paper").find("a").get_text(strip=True)
                soup2=get_soup(Article_link)
                DOI_element = soup2.find('div', class_='articleoverview-doi-wrap').find("a",class_="bluelink-style")

                if DOI_element:
                    DOI = DOI_element.get_text(strip=True).split("doi.org/")[-1]
                else:
                    DOI = ""
                pdf_link = soup2.find("div",class_="col-md-6 print-non-disp").find("a",class_="thirdlevel-pdf-btn").get("href")
