...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_2121.ini")
        ...
        text = en_soup.find('div', class_="ed-item web-55 no-padding").find("h2").get_text(strip=True)
        pattern = r"Volumen (\d+), No\. (\d+), Año (\d{4})"
        ...
        month_text = en_soup.find_all('h2')[1].get_text(strip=True)

        # Dictionary to convert Spanish months to English
        month_dict = {
        ...
        # Convert the extracted month to English
        month = month_dict.get(month_text, month_text)

        print(f"Volume: {volume}, Issue: {issue}, Year: {year},Month: {month}")

        All_articles = en_soup.find("div",class_="ed-item web-75 home-indice").find_all("li")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://www.alanrevista.org"+All_articles[article_index].find("a").get('href')

                Pdf_soup=get_soup(Article_link)
                Article_title =Pdf_soup.find("div",class_="h1").find("h1").get_text(strip=True)

                # Extracted page range
                page_range_text = Pdf_soup.find_all("p")[1].find("b").get_text(strip=True)
                pattern = r": (\d+-\d+)"
                ...
                if match:
                    ...
                    page_range=""
                try:
                    doi=Pdf_soup.find("div", class_="autores").find('a').get_text(strip=True).strip().split("doi.org/")[-1]
                    if doi:
                        print(f"DOI: {doi}")
                    else:
                        doi=""
                        print("DOI not found or is empty.")
                except Exception as e:
                    doi = ""
                    print(f"Failed to retrieve DOI: {str(e)}")


                pdf_link = Article_link.rstrip('/') + ".pdf"
