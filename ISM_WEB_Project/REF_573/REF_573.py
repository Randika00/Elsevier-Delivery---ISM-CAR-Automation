...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_573.ini")
        ...
        eng_href=soup.find('ul',class_="lang-nav").find('a').get('href')
        eng_soup=get_soup(eng_href)

        combine_soup=str(soup)+str(eng_soup)
        ...
        current_href=eng_soup.find('li',class_="preside-item").find('a').get('href')
        current_soup=get_soup(current_href)

        text = current_soup.find('section', class_="article-header").find("h1").get_text(strip=True)
        pattern = r"([A-Za-z]+)(?:-[A-Za-z]+)?\s+(\d{4})"
        ...
        All_articles = current_soup.find("ul",class_="index-row").find_all("a")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].get('href')

                pdf_page_soup=get_soup(Article_link)

                Article_title = pdf_page_soup.find("section", class_="article-header").find("h1").text.strip()
                article_page_href=pdf_page_soup.find("div", class_="article-header-icons").find('a').get('href')

                article_soup=get_soup(article_page_href)
                pdf_link = article_soup.find("a", class_="download").get('href')

                article_response = requests.get(pdf_link, headers=headers)

                # Read the PDF content using PyPDF2
                pdf_content = io.BytesIO(article_response.content)
                ...
                # Regular expression to extract the volume, issue, and page range
                pdf_pattern = r"(\d+)\(([\d\s–-]+)\):([\d–-]+)"

                pdf_match = re.search(pdf_pattern, pdf_text)

                if pdf_match:
                    volume = pdf_match.group(1)
                    issue = pdf_match.group(2).replace(" ", "").replace("–", "-")
                    page_range = pdf_match.group(3).replace("–", "-")

                    print(f"volume='{volume}', issue='{issue}', page_range='{page_range}'")
                else:
