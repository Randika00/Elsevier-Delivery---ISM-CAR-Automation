...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_355.ini")
        ...
        year_text = soup.find("h1").text.strip()
        try:
            month_text=soup.find("div",class_="col-md-9 col-xs-12").find("h2").text.strip()
            match = re.search(r'(\b[A-Za-z]+)\b.*(\b\d{4}\b)', month_text)
            ...
            print(f"month text change")

        match = re.search(r"(\d{4}) Vol\.(\d+), N°(\d+)(?: - Suppl\.(\d+))?", year_text)

        if match:
            year = match.group(1)
            volume = match.group(2)
            issue = match.group(3)
            supplement = match.group(4) if match.group(4) else ""

        else:
            print("The format of the text does not match the expected pattern.")

        print(f"year=\"{year}\",month=\"{month}\", volume=\"{volume}\", issue=\"{issue}\", supplement=\"{supplement}\"")

        All_articles = soup.find_all('div', class_='frarticoli')
        ...
        while article_index < len(All_articles):
            ...
            Article_title = None
            try:
                Article_title = All_articles[article_index].find('div', class_='art2').find('b').text.strip()
                page_range_element = All_articles[article_index].find('abbr', title="Page")
                page_range = page_range_element.parent.text.strip().split()[-1] if page_range_element else ""
                doi_element = All_articles[article_index].find('a', href=True, title="DOI")
                doi = doi_element.text.strip().split("doi.org/")[-1] if doi_element else ""

                print_bordered_message(f"Page Range: {page_range}, DOI: {doi}")

                pdf_link = "https://www.clinexprheumatol.org"+All_articles[article_index].find("div",class_="art4").find("a", title="DOWNLOAD PDF").get('href')
