...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_665.ini")
        ...
        date_time_meta=soup.find("div",class_="current_issue_title").get_text(strip=True)
        pattern = r'Vol\.\s*(\d+)\s*No\.\s*(\d+(?:\.\d+)?):\s*([a-zA-Z]+)\s*(\d{4})'
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find("h4", class_="title").find("a").get("href")
                Article_title = All_articles[article_index].find("h4", class_="title").find("a").get_text(strip=True)
                page_range= All_articles[article_index].find("div", class_="pages").get_text(strip=True)

                pdf_page_soup=get_soup(Article_link)

                try:
                    DOI_element = pdf_page_soup.find("span", class_="value").find("a")
                    if DOI_element:
                        doi = DOI_element.get_text(strip=True).split("doi.org/")[-1]
                    else:
                        doi = "DOI not found"
                except Exception as e:
                    doi = "DOI extraction error: " + str(e)

                try:
                    pdf_link_page = pdf_page_soup.find("a", class_="obj_galley_link pdf").get("href")
                    pdf_page_soup = get_soup(pdf_link_page)
                    pdf_link = pdf_page_soup.find("a", class_="download").get("href")
                except Exception as e:
                    pdf_link = "PDF link extraction error: " + str(e)
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    while retries < max_retries and not download_successful:
                        response = session.get(pdf_link, headers=headers, verify=False)
