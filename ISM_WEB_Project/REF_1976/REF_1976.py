...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_1976.ini")
        ...
        current_year_link=soup.find("td",class_="column-1").find("a").get("href")
        current_soup=get_soup(current_year_link)
        current_issue_link=current_soup.find_all("div",class_="elementor-button-wrapper")[-1].find("a").get("href")
        soup2=get_soup(current_issue_link)

        date_time_meta=current_soup.find_all("div",class_="elementor-button-wrapper")[-1].find("span",class_="elementor-button-text").get_text(strip=True)
        pattern = r"Volume\s(\d+),\sIssue\sNo\.\s(\d+),\s(\d+)"
        ...
        All_articles =soup2.find("tbody",class_="row-hover").find_all("tr")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find("td", class_="column-1").find("a").get("href")
                Article_title = All_articles[article_index].find("td", class_="column-1").find("a").get_text(separator=" ", strip=True)
                page_range=All_articles[article_index].find("td", class_="column-4").get_text(strip=True)
                pdf_link = All_articles[article_index].find("td", class_="column-1").find("a").get("href")
