...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_551.ini")
        ...
        date_time_meta=soup2.find("div",class_="col-md-12").get_text(strip=True)
        ...
        All_articles =soup2.find_all("div",class_="col-sm-6 col-md-6 col-lg-3 col-xl-3 col-xxl-3")
        Total_count = len(All_articles)
        print(f"Total number of articles: {Total_count}\n")

        article_index, article_check = 1, 0
        while article_index < len(All_articles):
            Article_count = article_index + 0
            Article_link, Article_title = None, None
            try:
                Article_link = All_articles[article_index].find("h5", class_="card-title").find("a").get("href")
                Article_title = All_articles[article_index].find("h5", class_="card-title").find("a").get_text(strip=True)

                soup3=get_soup(Article_link)
                DOI_element = soup3.find("div", class_="item doi").find("a")
                ...
                page_range_text=soup3.find("div",class_="csl-entry").get_text(strip=True)
                pattern = r"(\d{2}[-–]\d{2})"
                ...
                try:
                    pdf_link1 = soup3.find("a", class_="obj_galley_link pdf")["href"]
                    print(f'pdf_link1="{pdf_link1}"')
                    pdf_soup=get_soup(pdf_link1)
