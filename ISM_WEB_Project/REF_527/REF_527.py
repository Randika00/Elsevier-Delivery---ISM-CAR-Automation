...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_527.ini")
        ...
        CN_href = en_soup.find('div', class_="nav lf").find_all('a')[-1].get('href')
        ...
        All_articles =CN_soup.find("ul",class_="check_box").find_all("div",class_="slideTxtBox_list_right")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://jour.hhu.edu.cn/" + All_articles[article_index].find("div", class_="slideTxtBox_list_title").find("a").get("href")
                Article_title = All_articles[article_index].find("div", class_="slideTxtBox_list_title").find("a").get_text(strip=True)
                try:
                    time_text = All_articles[article_index].find("div", class_="slideTxtBox_list_time").i.text
                    time_pattern = r"(\d{4}),\s*(\d+)\((\d+)\):(\d+-\d+)"
                    time_match = re.search(time_pattern, time_text)
                    if time_match:
                        year = time_match.group(1)
                        volume = time_match.group(2)
                        issue = time_match.group(3)
                        page_range = time_match.group(4)
                    else:
                        year = volume = issue = page_range = None

                    doi = All_articles[article_index].find("a", class_="slide_red", href=True).get_text(strip=True)

                    print(f"volume='{volume}', issue='{issue}', year='{year}', page_range='{page_range}', doi='{doi}'")

                except Exception as e:
                    print(f"An error occurred: {e}")

                soup_2=get_soup(Article_link)
                pdf_link = soup_2.find("div",class_="p1 m-cell-hd").find("a",class_="pt1").get("href")

                if article_check == 0:
                    print(get_ordinal_suffix(Article_count) + " article details have been scraped")

                print_bordered_message(f"pdf_link:{pdf_link}")
