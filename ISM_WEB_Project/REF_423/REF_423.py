...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_423.ini")
        ...
        languages_tags=soup.find('div',{'class':'pkp_block block_language'})
        en_href=languages_tags.find("li",class_="locale_en_US").find('a').get('href')
        ru_href=languages_tags.find("li",class_="locale_uk_UA current").find('a').get('href')
        ...
        current_issue_link=en_soup.find('ul', class_='pkp_navigation_primary pkp_nav_list').find('li').find('a').attrs['href']
        current_issue_soup=get_soup(current_issue_link)
        en_href1 = current_issue_soup.find("li", class_="locale_en_US").find('a').get('href')
        en_soup2=get_soup(en_href1)

        input_string = en_soup2.find("h1").text.strip()
        ...
        All_articles =en_soup2.find_all('div', class_='obj_article_summary')
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                doi=article_soup.find("span", class_="value").find("a").text.strip().split("doi.org/")[-1]

                pdf_link_page = article_soup.find("a", class_="obj_galley_link pdf").get('href')
                pdf_soup=get_soup(pdf_link_page)
                pdf_link=pdf_soup.find("a", class_="download").get('href')
