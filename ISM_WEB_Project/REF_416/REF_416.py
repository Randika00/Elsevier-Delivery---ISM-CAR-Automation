...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_416.ini")
        ...
        text=soup.find("h3",class_="text-h2").get_text(strip=True)
        pattern = r"Volume\s(\d+)\sNumber\s(\d+)\s(\w+)\s(\d{4})"
        ...
        All_articles =soup.find_all("div",class_="media-body b-btm")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://www.tjpr.org/home/" + All_articles[article_index].find("a").get("href")
                Article_title = All_articles[article_index].find("h5", class_="text-h3").find("strong").get_text(strip=True)
                try:
                    DOI_element = All_articles[article_index].find("div", class_="text-h4").find_all("a")[-1]
                    ...
                    print(e)
                try:
                    pdf_link = f"https://www.tjpr.org/home/{All_articles[article_index].find_all("a")[1].get("href")}"
                except Exception as e:
                    print(e)
                if article_check == 0:
