...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_885.ini")
        ...
        All_articles =soup.find("div",class_="tab-content").find("div", class_="tab-pane fade articleListBox active in").find_all("div",class_="article-list-right")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://www.plantnutrifert.org" + All_articles[article_index].find("div", class_="article-list-title clearfix").find("a").get("href")
                Article_title = All_articles[article_index].find("div", class_="article-list-title clearfix").find("a").get_text(strip=True)
                try:
                    DOI_element = All_articles[article_index].find("div", class_="article-list-time").find("a")
                ...
                time_date_text = All_articles[article_index].find("div", class_="article-list-time").find("font").get_text(strip=True)

                pattern = r"(\d{4}),\s+(\d+)\((\d+)\):\s+\d+-\d+"
                match = re.search(pattern, time_date_text)
                ...
                try:
                    ...
                    page_range_match = re.search(r'(\d{4}-\d{4})', page_range_text)
                    page_range = page_range_match.group(1) if page_range_match else None
                except Exception as e:
                    print(f"An error occurred: {e}")
                try:
                    pdf_tag= All_articles[article_index].find("div", class_="article-list-zy article-list-btn clear").find("font",class_="font3").find('a', onclick=re.compile(r'downloadpdf'))
                    onclick_attr = pdf_tag['onclick']
                    pdf_id_match = re.search(r"downloadpdf\('(.+?)'\)", onclick_attr)
                    pdf_id = pdf_id_match.group(1) if pdf_id_match else None

                    if pdf_id:
                        print(f'Extracted PDF ID: {pdf_id}')
                    else:
                        raise ValueError("PDF ID not found")

                    pdf_link = f"https://www.plantnutrifert.org/article/exportPdf?id={pdf_id}&language=en"
                except Exception as e:
                    print(f"An error occurred: {e}")
                if article_check == 0:
