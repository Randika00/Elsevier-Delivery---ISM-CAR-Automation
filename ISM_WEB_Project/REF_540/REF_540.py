...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_540.ini")
        ...
        date_time_meta=soup.find("div",class_="new_number").find("h1").get_text(strip=True)
        pattern = r'Issue\s+(\d+)\((\d+)\),\s+(\d{4})'
        ...
        All_articles =soup.find_all("div",class_="block")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://www.vvmr.ru" + All_articles[article_index].find("label").find("a").get("href")
                Article_title = All_articles[article_index].find("label").find("a").get_text(strip=True)
                page_range=All_articles[article_index].find("span", class_="novan pages").get_text(strip=True)

                article_page=get_soup(Article_link)
                try:
                    DOI_element = article_page.find("div", class_="archive-doi").find("a")
                    ...
                    print("DOI not found")
                try:
                    pdf_link = f"https://www.vvmr.ru{article_page.find("div",class_="pdf-link").find("a").get("href")}"
                except Exception as e:
                    pdf_link =""
