...
chinese_url="http://www.gncl.cn/CN/current"
...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_369.ini")
        ...
        input_string = soup.find("h3", class_="latest-issue").text.strip()

        pattern = r"(\d+)\s+(\w+)\s+(\d+),\s+Volume\s+(\d+)\s+Issue\s+(\d+)"
        ...
        All_articles =soup.find_all('li', class_='noselectrow')
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                doi = All_articles[article_index].find("div", class_="j-volumn-doi").find("a", class_="j-doi").text.strip().split("doi.org/")[-1]
                page_range_string=All_articles[article_index].find("span", class_="j-volumn").text.strip()
                ...
                pdf_link = f"http://www.gncl.cn/EN/PDF/{doi}?token={token}"
