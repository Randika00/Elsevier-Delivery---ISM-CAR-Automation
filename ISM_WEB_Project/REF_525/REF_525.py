...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_525.ini")
        ...
        EN_link = "https://fnt.ilt.kharkiv.ua" + soup.find("a", class_="icon").get("href")
        ...
        date_time_meta=EN_soup.find("div",class_="current_issue_title").get_text(strip=True)
        pattern = r'Vol\. (\d+)\s+No\. (\d+)\s+\((\d{4})\)'
        # Match the pattern
        ...
        All_articles =EN_soup.find_all("div",class_="obj_article_summary")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    pdf_link_url=All_articles[article_index].find("a",class_="obj_galley_link pdf").get("href")
                    pdf_soup=get_soup(pdf_link_url)
                    pdf_link = pdf_soup.find("a",class_="download").get("href")
                except Exception as e:
                    pdf_link=""
                    print(f"pdf link is not available")
