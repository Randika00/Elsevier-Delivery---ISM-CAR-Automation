...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_635.ini")
        ...
        Ru_link = "http://journal.sfu-kras.ru" + soup.find("li", class_="item ru").find("a", class_="item-link").get("href")
        ...
        date_time_meta=soup.find("div",class_="journal-title").get_text(strip=True)
        pattern = r"(\d{4})\s+(\d+)\s+\((\d+)\)"
        ...
        All_articles =soup.find("ol",class_="articles").find_all("li")
        ...
        while article_index < len(All_articles):
            ...
            Article_title = None, None
            try:
                DOI = ""
                Article_title = All_articles[article_index].find("strong").get_text(strip=True)
                page_range_text=All_articles[article_index].find("div",class_="authors").get_text(strip=True)
                pattern = r"p\.\s*(\d+)[–—-](\d+)"
                ...
                pdf_link = All_articles[article_index].find("a",class_="link-text").get("href")
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    if download_successful:
                        ...
                        data.append({
                            ...
                            "Year": year, "URL": pdf_link, "SOURCE File Name": f"{pdf_count}.pdf",
                        ...
                        if pdf_link not in read_content:
                ...
                if article_check < 10:
                    ...
                    message = f"{pdf_link} - Error in {error_variable} for {Article_title}: [{str(error)}]"
