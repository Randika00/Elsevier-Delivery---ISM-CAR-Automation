...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_470.ini")
        ...
        current_issue_link=soup.find("div",class_="obj_issue_summary").find("a",class_="title").get("href")
        soup2=get_soup(current_issue_link)

        date_time_meta=soup2.find("h1",class_="issue_serie").get_text(strip=True)
        pattern = r"Vol\.\s*(\d+)\s*\((\d{4})\)"
        ...
        english_link=soup2.find("li",class_="languageToggleMenu__item").find("a").get('href')
        soup3=get_soup(english_link)

        All_articles =soup3.find_all("div",class_="obj_article_summary")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title = All_articles[article_index].find("div", class_="title").get_text(strip=True)

                soup3=get_soup(Article_link)

                try:
                    doi= soup3.find("div", class_="item doi").find("a").get_text(strip=True).split("doi.org/")[-1]
                except Exception as error:
                    error_list.append(f"Failed to send post request: {str(error)}")
                    print(error)

                try:
                    pdf_link_page = All_articles[article_index].find("a",class_="obj_galley_link pdf").get("href")
