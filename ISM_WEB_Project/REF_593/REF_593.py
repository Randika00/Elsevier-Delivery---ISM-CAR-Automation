...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_593.ini")
        ...
        soup = get_soup(url)
        date_text=soup.find("div",class_="currentIssueDate").get_text(strip=True)
        parts = date_text.split()
        month = parts[0]
        year = parts[1]
        print("Month:", month,"Year:", year)

        current_issue_link=soup.find('div', class_='menu').find_all('li')[3].find('a').get('href')
        current_issue_soup=get_soup(current_issue_link)

        All_articles =current_issue_soup.find("div",class_="articleBox").find_all('li')
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find("h2").find("a").get('href')
                Article_title = All_articles[article_index].find("h2").find("a").text.strip()
                meta_data_text=All_articles[article_index].find("div",class_="desc").find_all("p")[1].text.strip()
                pattern = r'Vol\. (\d+), Iss\. (\d+), pp\. (\d+-\d+)'

                # Search using the pattern
                match = re.search(pattern, meta_data_text)
                ...
                meta_links=All_articles[article_index].find_all("div",class_="desc")[1]

                doi=meta_links.find_all("a")[-1].get('href').split(".org/")[-1]
                pdf_link=meta_links.find('a', href=re.compile(r'\.pdf$'))['href']
