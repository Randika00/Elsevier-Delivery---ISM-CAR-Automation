...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_685.ini")
        ...
        current_issue_link=soup.find("h2",class_="identification").find("a").get("href")
        soup2=get_soup(current_issue_link)

        date_time_meta=soup2.find("h1",class_="issue-title page-title").get_text(strip=True)
        pattern = r"Vol\s+(\d+)\s*\((\w+)\s+(\d{1,2}),\s+(\d{4})\)"
        ...
        if match:
            ...
            date = match.group(3)
            year = match.group(4)
            print(f"volume={volume}, month={month}, date={date}, year={year}")
        else:
            print("No match found")
        All_articles =soup2.find_all("div",class_="article")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "https://tgh.amegroups.org" + All_articles[article_index].find("h5", class_="title").find("a").get("href")
                Article_title = All_articles[article_index].find("h5", class_="title").find("a",class_="title").get_text(strip=True)
                identifier = All_articles[article_index].find("div", class_="article_articleno").get_text(strip=True)

                pdf_article_soup=get_soup(Article_link)
                doi = pdf_article_soup.find("p", class_="doi").get_text(strip=True).split("doi: ")[-1].strip()

                try:
                    pdf_link_page = All_articles[article_index].find("a",class_="gallery").get("href")
                    pdf_link_soup=get_soup(pdf_link_page)
                    pdf_link=pdf_link_soup.find("a",class_="btn btn-primary",id="pdfDownloadLink").get("href")
                except Exception as error:
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    if download_successful:
                        ...
                        data.append({
                            ...
                            "Special Issue": "", "Page Range": "", "Month": month, "Day": date,
