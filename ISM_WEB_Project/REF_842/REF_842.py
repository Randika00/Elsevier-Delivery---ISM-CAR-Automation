...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_842.ini")
        ...
        soup=get_soup(url)
        # current_href=soup.find("ul",class_="nav navbar-nav").find("li").find("a").get("href")
        # soup_2=get_soup(current_href)

        date_time_meta=soup.find("p",class_="current_issue_title lead").get_text(strip=True)
        pattern = r"Vol\.\s*(\d+)\s*No\.\s*([\d–\-]+)\s*\((\d{4})\)"

        match = re.search(pattern, date_time_meta)
        if match:
            volume = match.group(1)
            issue = match.group(2).replace('–', '-')
            year = match.group(3)
        ...
        article_index, article_check = 8, 0
        while article_index < len(All_articles):
            Article_count = article_index + 1
            Article_link, Article_title = None, None
            try:
                Article_link = All_articles[article_index].find("h3", class_="media-heading").find("a").get("href")
                Article_title = All_articles[article_index].find("h3", class_="media-heading").find("a").get_text(strip=True)
                page_range=All_articles[article_index].find("p",class_="pages").get_text(strip=True)

                soup_3=get_soup(Article_link)

                try:
                    DOI_element = soup_3.find("div", class_="list-group-item doi").find("a")
                    if DOI_element:
                ...
                try:
                    link = soup_3.find("a", class_="galley-link btn btn-primary pdf").get("href")
                    pdf_link = link.replace('/view/', '/download/')
                except Exception as e:
                    print(f"An error occurred: {e}")
