...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_663.ini")
        ...
        date_time_meta=soup.find("h5").get_text(strip=True)

        if date_time_meta:
            parts = date_time_meta.split('|')[1].strip().split(', ')
            volume = parts[0].split()[-1]
            issue = parts[1].split()[-1]
            print(f'volume="{volume}", issue="{issue}"')
        else:
            print("No match found.")
        All_articles =soup.find_all("div",class_="post-text")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find('h5').find('a').get("href")
                Article_title = All_articles[article_index].find('h5').find('a').get('title')

                try:
                    page_range = All_articles[article_index].find('span', style="color:#a5a5a5").i.text.replace('Pages : ', '').strip()
                except AttributeError:
                    page_range = None

                try:
                    doi_tag = All_articles[article_index].find('a', href=re.compile(r'https?://dx.doi.org/'))
                    if doi_tag:
                        doi_full = doi_tag.get('href')
                        doi = doi_full.split('https://dx.doi.org/')[1]
                    else:
                        print("DOI not found")
                except AttributeError:
                    doi = None
                ...
                try:
                    pdf_link = All_articles[article_index].find('a', string='PDF')['href']
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    if download_successful:
                        ...
                        data.append({
                            ...
                            "Year": "", "URL": Article_link, "SOURCE File Name": f"{pdf_count}.pdf",
                            "user_id": user_id
                        })
