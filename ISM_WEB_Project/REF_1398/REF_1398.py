...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ...
            ini_path = os.path.join(os.getcwd(), "REF_1398.ini")
        ...
        date_time_meta=soup.find("li",class_="current").find("h1").get_text(strip=True)
        pattern = r"Vol\s(\d+)\sNo\s(\d+)\s\((\d{4})\):\s([A-Za-z]+)\s(\d{4})"
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    DOI_element = soup2.find("span", class_="value").find("a")
                ...
                try:
                    pdf_page_link = soup2.find("a",class_="obj_galley_link pdf").get("href")
