...
url = "https://lockwoodonlinejournals.com/index.php/jaos"
try:
    page = requests.get(url, headers=headers)
    soup = BeautifulSoup(page.content, 'html.parser')
    element = soup.find("p", class_="current_issue_title lead").get_text(strip=True)
    volume, issue, year = extract_info(element)
    all_articles = soup.find_all("div", class_="article-summary media")
    Total_count = len(all_articles)
    print(f"Total number of articles:{Total_count}", "\n")
    for article in all_articles:
        Article_link = None
        try:
            Article_link = article.find("h3", class_="media-heading").find("a").get("href")
            title = article.find("h3", class_="media-heading").find("a").get_text(strip=True)
            ...
            doi_string = Article_link_soup.find("div", class_="list-group-item doi myDownload").find("a").get_text(strip=True)
            parts = doi_string.split('/')
            doi = "/".join(parts[3:])
            pdf_url = Article_link_soup.find("div", class_="download myDownload").find("a").get("href")
