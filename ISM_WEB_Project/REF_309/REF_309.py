...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_309.ini")
        ...
        soup = get_soup(url)
        ch_url = "http://zgspxb.cnjournals.org" + soup.find('div', class_="nav").find("ul").find_all("li")[-1].find("a").get("href")
        ...
        year_text = soup.find("span", class_="banTitle").text.strip()

        match = re.search(r"Volume (\d+),(\d+) Issue (\d+)", year_text)
        if match:
            volume = match.group(1)
            year = match.group(2)
            issue = match.group(3)
            print_bordered_message(f"Volume: {volume}, Year: {year}, Issue: {issue}")
        else:
            print("The format of the text does not match the expected pattern.")

        All_articles = soup.find("ul",class_="check_box").find_all('div', class_='slideTxtBox_list_right slideOnDiv')
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = "http://zgspxb.cnjournals.org" + All_articles[article_index].find("div", class_="slideTxtBox_list_title").find('a').get('href')
                Article_title = All_articles[article_index].find("div", class_="slideTxtBox_list_title").find('a').text.strip()
                page_range_text = All_articles[article_index].find("div", class_="slideTxtBox_list_time").find('i', class_="slide").text.strip()
                ...
                # Extract the DOI
                doi = All_articles[article_index].find('a', class_='slide_red').text.strip()
                print_bordered_message(f"Page Range: {page_range}, DOI: {doi}")

                pdf_link = "http://zgspxb.cnjournals.org/" + All_articles[article_index].find("div", class_="slideTxtBox_list_zy").find_all("i")[-1].find('a').get('href')
