...
try:
    url = "https://jelectrochem.xmu.edu.cn/journal/"

    current_datetime = datetime.now()
    current_date = str(current_datetime.date())
    current_time = current_datetime.strftime("%H:%M:%S")

    ini_path = os.path.join(os.getcwd(), "Ref_112.ini")
    ...
    for single_element in div_element:
        ...
        try:
            ...
            volume_issue_info = soup.find("h1").text.strip()
            ...
            if article_response.status_code == 200:
                ...
                doi_tag = article_soup.find("div", id="alpha")
                if doi_tag:
                    doi_tag = doi_tag.find("div", id="doi_link")
                    if doi_tag:
                        doi_tag = doi_tag.find("p")
                        if doi_tag and doi_tag.find("a"):
                            doi_link = doi_tag.find("a")["href"]
                            doi = re.search(r"10.\d{4,9}/[-._;()/:A-Z0-9]+", doi_link, re.IGNORECASE)
                            if doi:
                                doi = doi.group()
                            else:
                                doi = "DOI not found"
                        ...
                        doi = "DOI link not available"
                else:
                    doi = "DOI alpha section not available"
