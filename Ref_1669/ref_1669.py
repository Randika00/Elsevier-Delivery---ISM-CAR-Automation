...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://umo.abvpress.ru/"
            toc_url = "https://umo.abvpress.ru/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_1669.ini")
            ...
            archive_section = soup.find("div", id="issues").find("div", class_="archive")

            all_issues = archive_section.find_all("div", id="issue")
            latest_issue = all_issues[-1]

            link = latest_issue.find("a")["href"]
            link_id = link.split('/')[-2]

            current_link = f"https://umo.abvpress.ru/jour/user/setLocale/en_US?source=%2Fjour%2Fissue%2Fview%2F{link_id}%2FshowToc"

            volume_issue_text = latest_issue.find("h4").text.strip()

            volume = volume_issue_text.split(",")[0].split(" ")[1]
            issue = volume_issue_text.split(",")[1].split(" ")[2]
            year = volume_issue_text.split("(")[1].strip(")")
                        ...
                        if article_title_div:
                            ...
                            if last_response.status_code == 200:
                                ...
                                if article_response.status_code == 200:
                                    ...
                                    try:
                                        if doi_tag:
                                            doi_anchor = doi_tag.find("a", id="pub-id::doi")
                                            if doi_anchor:
                                                doi = doi_anchor.get('href').replace("https://doi.org/", "") if doi_tag else ""
