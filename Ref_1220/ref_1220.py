...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://hsm.susu.ru"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_1220.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="issues_archive hsm-issue-container").find("div", class_="obj_issue_summary").find("a", class_="cover").get("href")
            link_text = base_url + link
            link_id = link_text.split('/')[-1]

            current_link = f"https://hsm.susu.ru/hsm/user/setLocale/en_US?source=%2Fhsm%2Fen%2Fissue%2Fview%2F{link_id}"
                            ...
                            if last_page_text:
                                ...
                                if last_response.status_code == 200:
                                    ...
                                    if article_response.status_code == 200:
                                        ...
                                        doi_tag = article_soup.find("div", class_="item doi")
                                        doi = ""
                                        if doi_tag:
