...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_3721.ini")
        ...
        CN_href=en_soup.find('ul',class_="nav navbar-nav").find_all('a')[-1].get('href')
        ...
        text = en_soup.find('div', class_="n-j-q").get_text(strip=True)
        pattern = r'(\d+) (\w+) (\d+), Volume (\d+) Issue (\d+)'
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find("div", class_="j-title-1 biaotih").find("a").get('href')
                Article_title = All_articles[article_index].find("div", class_="j-title-1 biaotih").find("a").get_text(strip=True)
                ...
                doi=All_articles[article_index].find("div", class_="j-volumn-doi").find('a',class_="j-doi").get_text(strip=True).split("doi.org/")[-1]

                token = get_token(url)
                print(token)

                pdf_link = f"https://manu60.magtech.com.cn/biotech/EN/PDF/{doi}?token={token}"
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    while retries < max_retries and not download_successful:
                        response = session.get(pdf_link,proxies=get_random_proxy(), headers=headers)
