...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.internationaljournalssrg.org/IJEEE/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_396.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="cat").findAll("li")[3].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("table", class_="table table-bordered").findAll("tr")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("span")
                        if article_title_div:
                            ...
                            volume_issue_month_info = soup.find("div", class_="post-heading").find("h1")
                            volume, issue, month, year = "", "", "", ""
                            if volume_issue_month_info:
                                volume_issue_month_text = volume_issue_month_info.text.strip()
                                match = re.search(r'Volume\s+(\d+)\s+Issue\s+(\d+)\s+-\s+([\w\s]+)\s+(\d+)', volume_issue_month_text)

                                if match:
                                    volume = match.group(1) if match else ""
                                    issue = match.group(2) if match else ""
                                    month = match.group(3) if match else ""
                                    year = match.group(4) if match else ""

                                else:
                                    print("Could not extract volume, issue, month, or year.")

                            article_response = get_response_with_retry(article_link, headers=headers)
                            if article_response.status_code == 200:
                                article_soup = BeautifulSoup(article_response.content, 'html.parser')

                                pages_info = article_soup.find("p")
                                page_range = ""
                                if pages_info:
                                    text = pages_info.get_text()
                                    match = re.search(r'pp\.\s*(\d+)-(\d+)', text)
                                    if match:
                                        page_start = match.group(1)
                                        page_end = match.group(2)
                                        page_range = f"{page_start}-{page_end}" if pages_info else ""
                                    else:
                                        print("Could not extract page range.")
                                else:
                                    print("No <p> tag found.")

                                doi_tags = article_soup.find("div", class_="doi")
                                doi = ""
                                if doi_tags:
                                    doi = doi_tags.find("a").get("href").replace("https://doi.org/", "") if doi_tags else ""

                                pdf_url_info = article_soup.find("div", class_="span2")
                                pdf_link = ""
                                if pdf_url_info:
                                    pdf_link = pdf_url_info.find("a").get('href') if pdf_url_info else ""
