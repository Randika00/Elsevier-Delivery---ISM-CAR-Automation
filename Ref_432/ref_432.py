...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.ejournal.org.cn/EN/home"
            toc_url = "https://www.ejournal.org.cn/EN/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_432.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", id="myTab", class_="nav nav-tabs article-list nav-tabs_en").findAll("li")[1].find("a").get("href")
            link_tag = base_url + link
            current_link = link_tag.replace('/home#mytab1', '/current')
                        ...
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                if latest_issue_element:
                                    ...
                                    token = get_token("http://www.ejournal.org.cn/")
                                    pdf_link = f"https://www.ejournal.org.cn/EN/PDF/{doi}?token={token}"
