...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://mathematics-vestnik.ksu.kz"
            toc_url = "https://mathematics-vestnik.ksu.kz/index.php/mathematics-vestnik/user/setLocale/en_US?source=%2Findex.php%2Fmathematics-vestnik%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_510.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="issues_archive").find("div", class_="obj_issue_summary").find("a", class_="title")
            link_text = link["href"]
            link_id = link_text.split('/')[-1]

            text = link.get_text(strip=True)
            year, volume, issue = text.split()
            volume = volume.strip("()")

            current_link = f"https://mathematics-vestnik.ksu.kz/index.php/mathematics-vestnik/user/setLocale/en_US?source=%2Findex.php%2Fmathematics-vestnik%2Fissue%2Fview%2F{link_id}"
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            doi_tag = single_element.find("span", class_="doiInSummary")
                            doi = ""
                            if doi_tag:
                                link_tag = doi_tag.find("a")
                                if link_tag and link_tag.get("href"):
                                    doi = link_tag["href"].replace("https://doi.org/", "") if doi_tag else ""
                                else:
                                    doi = "DOI not found"
                            else:
                                doi = "DOI not found"
