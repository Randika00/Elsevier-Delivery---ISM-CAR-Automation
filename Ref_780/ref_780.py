...
def get_response_with_retry(url, headers, retries=5, backoff_factor=1, status_forcelist=(500, 502, 504)):
    ...
    response = session.get(url, headers=headers, timeout=60)  # Increased timeout
    return response
...
try:
    ...
    for line in url_details:
        try:
            ...
            toc_url = "http://rgjtxb.jtxb.cn/EN/1000-985X/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_780.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="nav navbar-nav").find("ul", class_="dropdown-menu").findAll("li")[0].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="content_nr").findAll("div", class_="wenzhang")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_={"dqml_wz", "dqml_qbwz"})
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                date_volume_issue_info = soup.find("div", class_="qknr")
                                day, month, year, volume, issue = "", "", "", "", ""
                                if date_volume_issue_info:
                                    date_volume_issue_text = date_volume_issue_info.get_text(strip=True)

                                    date_volume_issue_match = re.search(r'(\d{1,2}) (\w+) (\d{4}), Volume (\d+) Issue (\d+)', date_volume_issue_text)
                                    ...
                                    for link in pdf_links:
                                        ...
                                        if pdf_id_match:
                                            ...
                                            base_pdf_url = "http://rgjtxb.jtxb.cn/EN/article/downloadArticleFile.do?attachType=PDF&id="
