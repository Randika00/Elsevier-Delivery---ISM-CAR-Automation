...
def extract_month_from_pdf(pdf_file_path):
    try:
        with open(pdf_file_path, 'rb') as file:
            ...
            match = re.search(r'\|\s*(\w+)\s+\d{4}\s+\|\s*Volume', first_page_text)
            if match:
                return match.group(1)
            else:
                print("Month not found in the PDF.")
                return None
    except Exception as e:
...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.biophysics-reports.org/en/article/"
            toc_url = "http://www.biophysics-reports.org/en/article/current"
            article_div = "http://www.biophysics-reports.org"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_3803.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("ul", class_="tab-ul tab-ul-index tab-ul-index-en clearfix").find("li", class_="current_ active").find("a").get("href").replace("#", "")
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", id="issueList").findAll("div", class_="article-list")
                ...
                for single_element in div_element:
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            page_range, doi = "", ""
                            if pages_doi_tag:
                                page_range = pages_doi_tag.find_all("font")[0].text.split(":")[1].strip().rstrip(
                                    ".") if pages_doi_tag else ""
                                doi = pages_doi_tag.find("a").get("href").replace("/article/doi/", "") if pages_doi_tag else ""
                            ...
                            pdf_url = f"http://www.biophysics-reports.org/article/exportPdf?id={id_values_string}&language=en"

                            pdf_file_path = os.path.join(current_out, f"{pdf_count}.pdf")

                            month = ""
                            if download_pdf(pdf_url, pdf_file_path):

                                month = extract_month_from_pdf(pdf_file_path)
                                # if month:
                                #     print(f"Month extracted from the PDF: {month}")

                                completed_list.append(pdf_file_path)
                ...
                try:
                    ...
                    try:
                        ...
                        error_message = f"Error processing Chinese version of URL {url_cn}: {str(e)}"
                        error_list.append(error_message)
                        print(error_message)
            ...
            print(Error_message)
            error_list.append(Error_message)
            common_function.attachment_for_email(source_id, duplicate_list, error_list, completed_list, len(completed_list), ini_path, attachment, current_date, current_time, Ref_value)
