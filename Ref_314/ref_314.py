...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://xdspkj.ijournals.cn/"
            toc_link = "https://xdspkj.ijournals.cn/xdspkjen/home"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_314.ini")
            ...
            link_tag = soup.find("div", class_="slideBox_info").find("div", class_="slideBox_nr").find("a")

            link = link_tag["href"]
            current_link = base_url + link

            match = re.search(r'(\d{4})_(\d+)_(\d+)', link)
            ...
            toc_url = f"https://xdspkj.ijournals.cn/xdspkjen/article/issue/{year}_{volume}_{issue}"
            ...
            if response.status_code == 200:
                ...
                div_element = soup.find("div", class_="article_search_form").findAll("div", class_="article_list")
                ...
                for single_element in div_element:
                    ...
                    try:
                        article_title_div = single_element.find("div", class_="article_list_title")
                        if article_title_div:
                            ...
                            token = single_element.find("div", class_="article_list_left")
                            id_value = ""
                            if token:
                                input_tag = token.find("input")
                                if input_tag and "value" in input_tag.attrs:
                                    id_value = input_tag["value"]

                            pages_doi_tag = single_element.find("div", class_="article_list_time")
                            page_range, doi = "", ""
                            if pages_doi_tag:
                                page_ranges_doi_text = pages_doi_tag.get_text(strip=True)
                                match = re.search(r'\((\d+)\):(\d+-\d+)\.DOI:(\S+)', page_ranges_doi_text)

                                if match:
                                    page_range = match.group(2) if match else ""
                                    doi = match.group(3) if match else ""
                                else:
                                    print("No pages, doi data found!")

                            pdf_url = f"https://xdspkj.ijournals.cn/xdspkjen/article/pdf/{id_value}?st=article_issue"

                            link_text = single_element.find("div", class_="article_list_abstract").find("font", class_="pdf_url")
                            pdf_url = ""
                            if link_text:
                                pdf_links = link_text.find("a").get('href')
                                pdf_url = urllib.parse.urljoin(base_url, pdf_links)
                ...
                try:
                    ...
                    try:
                        url_cn = toc_url.replace('/xdspkjen', '/xdspkj')
