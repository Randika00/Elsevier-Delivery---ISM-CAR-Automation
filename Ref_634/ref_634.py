...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://j.bjfu.edu.cn"
            toc_url = "http://j.bjfu.edu.cn/en/article/current"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_634.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", id="archive").find("div", class_="arci-t pull-left")
            if link:
                anchor = link.find("a")
                if anchor and anchor.get("href"):
                    link_text = anchor["href"]
                    current_link = base_url + link_text
                    match = re.search(r"/(\d{4})/(\d+)", link_text)
                    year, issue = "", ""
                    if match:
                        year = match.group(1)
                        issue = match.group(2)
                    else:
                        print("No year issue data found")
                        ...
                        for single_element in div_element:
                            ...
                            try:
                                ...
                                if article_title_div:
                                    ...
                                    pdf_link = f"http://j.bjfu.edu.cn/article/exportPdf?id={id_values_string}&language=en"
                                    ...
                                    month_info = article_soup.find("div", class_="toggle-author-wrap").find_all("li")[4]
                                    ...
                                    if Check_duplicate.lower() == "true" and check_value:
                                        ...
                                        data.append(
                                            {"Title": article_title, "DOI": doi, "Publisher Item Type": "",
                                             ...
                                             "Special Issue": "", "Page Range": page_range, "Month": "March",
                                             "Day": "",
                                             "Year": year,
