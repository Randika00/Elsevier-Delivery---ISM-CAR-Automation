...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://nytsqb.aiijournal.com/EN"
            toc_url = "http://nytsqb.aiijournal.com/EN/1002-1248/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_493.ini")
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                pattern = r"(\d+-\d+)\.\s*DOI:(10\.\d+\/[^\s]+)"
                                match = re.search(pattern, page_ranges_doi_text)

                                if match:
                                    page_range, doi = match.groups()
                                else:
                                    print("Page Range, DOI not found.")
                            ...
                            token = get_token("http://nytsqb.aiijournal.com/EN/1002-1248/home.shtml")
                            pdf_link = f"http://nytsqb.aiijournal.com/EN/PDF/{doi}?token={token}"
