...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.epae.cn/dlzdhsb/ch/"
            toc_url = "https://www.epae.cn/dlzdhsb/ch/index.aspx"
            ...
            ini_path = os.path.join(os.getcwd(), "Ref_455.ini")
            ...
            link = BeautifulSoup(response.content, "html.parser").find("div", class_="Cimg1").find("div", class_="fm1").find("a").get("href")
                    ...
                    try:
                        ...
                        for tr in article_title_div:
                            ...
                            if link_tag:
                                ...
                                if article_link not in printed_links:
                                    printed_links.add(article_link)

                                    volume_info = single_element.findAll("tr")
                                    volume = ""
                                    td_elements = volume_info[4].find_all("td")
                                    text = td_elements[1].get_text(strip=True)
                                    match = re.search(r"\d{4},(\d+)\(\d+\):", text)
                                    if match:
                                        volume = match.group(1) if text else ""
                                    else:
                                        print("Could not extract volume details.")
                                    ...
                                    if article_response.status_code == 200:
                                        ...
                                        article_text = article_soup.find("td", {"align": "center", "class": "intro", "style": "font-family:times new roman ", "width": "100%"})
                                        article_title = ""
                                        if article_text:
                                            article_title = article_text.find("span", id="EnTitle").text.strip() if article_text else ""
                                        ...
                                        year_volume_info = article_soup.find("table", {"border": "0", "cellpadding": "4", "cellspacing": "4", "width": "100%"}).find_all("tr")[1]
                                        year, issue, page_range = "", "", ""
                                        if year_volume_info:
                                            text = year_volume_info.find_all("td")[1].text
                                            match = re.search(r"(\d{4}),\d+\((\d+)\):([\d\-,]+)", text)
                                            if match:
                                                year = match.group(1) if text else ""
                                                issue = match.group(2) if text else ""
                                                page_range = match.group(3) if text else ""
                                            else:
                                                print("No match found in the text")
                                        else:
                                            print("year, issue and page range info not found")
