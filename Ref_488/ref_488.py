...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://medtech.mphu.edu.ua/"
            toc_url = "https://medtech.mphu.edu.ua/user/setLocale/en_US?source=%2Fissue%2Fcurrent"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_488.ini")
            ...
            link_text = link.find("a", class_="cover")["href"]
            link_id = link_text.split('/')[-1]

            current_link = f"https://medtech.mphu.edu.ua/user/setLocale/en_US?source=%2Fissue%2Fview%2F{link_id}"

            h2_text = link.find("h2").find("div", class_="series").text.strip()
            volume, issue, year = "", "", ""

            if "Vol." in h2_text and "No." in h2_text:
                parts = h2_text.split()
                volume = parts[1]
                issue = parts[3].strip("()")
                year = parts[-1].strip("()")
