...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            ini_path = os.path.join(os.getcwd(), "REF_222.ini")
        ...
        text=soup.find('span',class_="dateIssuehead border-btm").get_text(strip=True)
        pattern = r"(\d{2}) (\w+) (\d{4}) \| Vol (\d+), Issue (\d+)"
        ...
        panel_blocks=soup.findAll("div",class_="panelBlock")
        h4_tags = []
        for panel in panel_blocks:
            h4_tags.extend(panel.find_all('h4'))

        Total_count=len(h4_tags)
        print(f"Total number of articles:{Total_count}","\n")

        article_index, article_check = 0, 0
        while article_index < len(h4_tags):
            Article_count = article_index+1
            Article_link, Article_title = None, None
            try:
                Article_title=h4_tags[article_index].find("a",class_="processed").text.strip()
                Article_link="https://journals.rcni.com"+h4_tags[article_index].find("a",class_="processed").get("href")

                soup2=get_soup(Article_link)

                try:
                    doi_text = soup2.find("div", class_="articleLeftBlock").find_all("p", class_="mar-bot-15")
                    doi=next((ele.get_text(strip=True) for ele in doi_text if "doi:" in ele.get_text(strip=True)),None)
                    pattern = r"doi:\s*(\S+)"
                ...
                pdf_link="https://journals.rcni.com"+soup2.find("div",class_="articleAction").find("a",class_="txtWrp wrapicon").get("href")
