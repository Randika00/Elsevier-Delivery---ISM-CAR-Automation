...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.informatica.si/"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_1006.ini")
            ...
            issue_div = soup.find("div", id="issues").find("div", id="issue")

            link_tag = issue_div.find("h4").find("a")
            link_url = link_tag["href"]
            title_text = link_tag.text

            volume, issue, year = "", "", ""
            match = re.search(r"Vol (\d+), No (\d+) \((\d+)\)", title_text)
            if match:
                ...
                if article_response.status_code == 200:
                    ...
                    content_div = link_soup.find("div", id="content").find("ul", class_="menu")
                    current_link = ""
                    if content_div:
                        current_link = content_div.find("a").get("href")
                        ...
                        if response.status_code == 200:
                            ...
                            for single_element in div_element:
                                ...
                                try:
                                    article_title_div = single_element.find("td", class_="tocTitle")
                                    if article_title_div:
                                        article_title = article_title_div.find("a").text.strip()
                                        article_link = article_title_div.find("a").get('href')

                                        last_page_text = single_element.find("td", class_="tocGalleys")
                                        last_page_link = ""
                                        if last_page_text:
