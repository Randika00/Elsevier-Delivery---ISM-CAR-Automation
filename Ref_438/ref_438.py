...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "http://www.fzxb.org.cn/EN"
            toc_url = "http://www.fzxb.org.cn/EN/0253-9721/current.shtml"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_438.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("a", string="Current Issue").get("href")
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if page_ranges_doi_tag:
                                ...
                                doi_tag = single_element.find("dd", class_="kmnjq").find("a", href=True)
                                doi = ""
                                if doi_tag:
                                    doi = doi_tag.get("href").split("doi.org/")[-1] if doi_tag else ""

                                date_volume_issue_info = soup.find("div", class_="njq")
                                day, month, year, volume, issue = "", "", "", "", ""
                                if date_volume_issue_info:
                                    text_content = date_volume_issue_info.get_text(strip=True)

                                    match = re.search(r'(\d{1,2})\s(\w+)\s(\d{4}),\sVolume\s(\d+)\sIssue\s(\d+)', text_content)

                                    if match:
                                        day = match.group(1) if text_content else ""
                                        month = match.group(2) if text_content else ""
                                        year = match.group(3) if text_content else ""
                                        volume = match.group(4) if text_content else ""
                                        issue = match.group(5) if text_content else ""

                                token = get_token("http://www.fzxb.org.cn/EN/0253-9721/home.shtml")
                                pdf_link = f"http://www.fzxb.org.cn/EN/PDF/{doi}?token={token}"
