...
try:
    ...
    for line in url_details:
        try:
            ...
            base_url = "https://www.ejcrim.com/index.php"

            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Ref_895.ini")
            ...
            current_link = BeautifulSoup(response.content, "html.parser").find("ul", class_="pkp_navigation_primary pkp_nav_list", id="navigationPrimary").findAll("li")[7].find("a").get("href")
            ...
            if response.status_code == 200:
                ...
                all_articles = soup.find("ul", class_="articles").findAll("table", class_="TOCarticle")

                div_element = soup.find("ul", class_="articles").findAll("li")
                    ...
                    try:
                        ...
                        if article_title_div:
                            ...
                            if year_volume_issue_info:
                                volume_issue_year_text = year_volume_issue_info.text.strip()

                                pattern = r"(\d{4}): Vol (\d+) No (\d+)"
                                match = re.search(pattern, volume_issue_year_text)
                                if match:
                                    year = match.group(1) if volume_issue_year_text else ""
                                    volume = match.group(2) if volume_issue_year_text else ""
                                    issue = match.group(3) if volume_issue_year_text else ""
                                else:
