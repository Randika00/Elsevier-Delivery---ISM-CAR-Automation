...
while url_index < len(url_list):
    try:
        ...
        if url_check == 0:
            print(url_id)
            ini_path = os.path.join(os.getcwd(), "REF_3872.ini")
        ...
        text = en_soup.find('div', class_="njq").get_text(strip=True)
        ...
        All_articles = en_soup.find_all("div",class_="noselectrow")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link = All_articles[article_index].find("div", class_="col-lg-9 col-md-9 col-sm-9 dqml_wz").find("a").get('href')
                Article_title = All_articles[article_index].find("div", class_="col-lg-9 col-md-9 col-sm-9 dqml_wz").find("a").get_text(strip=True)
                meta_data=All_articles[article_index].find("dd", class_="kmnjq").get_text(strip=True)
                pattern = r'\d+\(\d+\):\s*([\d\-]+)\s*\.'
                ...
                try:
                    doi=All_articles[article_index].find("dd", class_="kmnjq").find('a').get_text(strip=True)
                    if doi:
                        print(f"DOI: {doi}")
                    else:
                        doi=""
                ...
                pdf_link = f"http://fcst.ceaj.org/EN/PDF/{doi}?token={token}"
